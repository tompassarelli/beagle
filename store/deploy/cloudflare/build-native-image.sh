#!/usr/bin/env bash
# The Cloudflare Beagle Store server image: this checkout's static native artifact
# packaged into a scratch runtime image. stdout is only the image tag:
#   export BEAGLE_STORE_SERVER_IMAGE="$(deploy/cloudflare/build-native-image.sh)"
set -euo pipefail
...
cc_link="${BEAGLE_STORE_NATIVE_CC_LINK:-${HOME:?}/.cache/store/native-build/.musl-cc}"
image_name="${BEAGLE_STORE_SERVER_IMAGE_NAME:-beagle-store-server-native}"

cc="${BEAGLE_STORE_NATIVE_CC:-}"
if [[ -z "$cc" ]]; then
...
artifact="$(
  BEAGLE_STORE_NATIVE_CC="$cc" BEAGLE_STORE_NATIVE_STATIC=1 \
    "$repo/bin/beagle-store-native-build" --host server "${sources[@]}"
)"
tag="$image_name:${artifact##*/}"
"$repo/bin/beagle-store-cloudflare-native-image" --artifact "$artifact" --tag "$tag" >&2
