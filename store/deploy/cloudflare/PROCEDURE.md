...
Worker talks to over the network. Beagle Store also links as a wasm module an isolate
...
The Beagle Store server image is one static native executable on `scratch` — no JVM, no
dynamic loader, no shell. It is packaged from a completed content-addressed
`beagle-store-native-build` artifact, so the image tag names the exact artifact it
carries. The separate shim image remains Babashka.

From `store:deploy/cloudflare`:

```sh
export BEAGLE_STORE_SERVER_IMAGE="$(./build-native-image.sh)"
export SHIM_TOKEN=$(openssl rand -hex 32)
export BEAGLE_STORE_SPACE_ID=my-production-space
...
`BEAGLE_STORE_NATIVE_STATIC=1` is what does that: it appends `-static` to the server
...
```sh
artifact="$(BEAGLE_STORE_NATIVE_CC="$cc" BEAGLE_STORE_NATIVE_STATIC=1 \
  bin/beagle-store-native-build --host server "${sources[@]}")"
bin/beagle-store-cloudflare-native-image --artifact "$artifact" --tag "beagle-store-server-native:${artifact##*/}"
```
...
producing the `scratch` runtime image, and sets `BEAGLE_STORE_BIND=0.0.0.0` because
...
operation to the wrong path is rejected before Beagle Store server contact.

## Deploy the Worker

Set `SHIM_URL` and `BEAGLE_STORE_SPACE_ID` in `store:deploy/cloudflare/wrangler.toml`,
...
```sh
W=https://store-bench.example.workers.dev
curl -sS "$W/health"
