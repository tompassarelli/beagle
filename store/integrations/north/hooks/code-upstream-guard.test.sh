...
# that a denial redirects to the Beagle Store graph-edit tools.
set -uo pipefail
...
printf '%s\n' ';; @upstream:graph (managed by store — do not edit as text)' '(defn a [] 1)' >"$SENTINEL_SUFFIX"
...
assert "mcp__store__" in h["permissionDecisionReason"]
...
# denial guidance names the Beagle Store graph-edit verbs explicitly.
run_hook 0 "$(event Edit "$SENTINEL_CANON")"
if [[ "$RUN_OUT" == *'mcp__store__set-body'* && "$RUN_OUT" == *'mcp__store__rename-def'* ]]; then
  ok 'denial reason redirects to Beagle Store graph-edit tools'
else
  not_ok "denial reason redirects to Beagle Store graph-edit tools (got: $RUN_OUT)"
fi
