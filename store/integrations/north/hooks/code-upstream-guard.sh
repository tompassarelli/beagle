...
# ============================================================================
# STATUS: OWNER COPY. Beagle Store owns this integration at
# `store:integrations/north/hooks/code-upstream-guard.sh`; North composes it into
...
#         (default: ~/.config/store/graph-upstream-files). A row is matched by
...
REGISTRY="${GRAPH_UPSTREAM_REGISTRY:-$HOME/.config/store/graph-upstream-files}"
...
This file's UPSTREAM is the GRAPH: the code lives in the Beagle Store graph — recursive
...
EDIT via the store MCP tools instead:
  - mcp__store__add-def     — add a new top-level def (upsert-form, new name)
  - mcp__store__set-body    — replace a defn's body
  - mcp__store__rename-def  — rename a def (O(1), scope-correct via refers_to)
...
# by store)`). The marker must be anchored at the start of the comment payload and
