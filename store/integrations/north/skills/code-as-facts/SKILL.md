...
"Facts" here is the view-level sense (store:docs/ontology.md): the selected live
...
of truth is the **Beagle Store fact graph**, and the on-disk `.bclj` is a *regenerated
...
- its absolute path is listed in `$GRAPH_UPSTREAM_REGISTRY`
  (default `~/.config/store/graph-upstream-files`) — the authoritative marker, OR
...
The authoring engine is `store:resolve.clj` (modes
`upsert-form` / `set-body` / `rename` / `delete`), exposed AI-facing over the store
...
| Intent | Tool | Notes |
|---|---|---|
| Add a new top-level def | `mcp__store__add-def` | `upsert-form` with a new name; appends a wrapper `fN` edge |
| Replace a def by name | `mcp__store__add-def` | `upsert-form` with an existing name; withdraws its live wrapper `fN` assertion, then asserts the replacement edge |
| Replace a defn's body | `mcp__store__set-body` | withdraws the live body-slot `fN` assertion(s), then asserts the newly minted body at the first body slot |
| Rename a def | `mcp__store__rename-def` | O(1), scope-correct via `refers_to`, shadow-safe |
| Insert a form after an anchor | `mcp__store__insert-after` | ordered placement |
| Insert any valid top-level form before a named def | `mcp__store__insert-before` | ordered wrapper edge; candidate compilation must pass |
...
splice. It is minted into the same Beagle Store store as `kind`/`v`/`fN` facts, and any
...
> If the guard denies and the `mcp__store__*` verbs are somehow absent, surface
> the gap — never fall back to text Edit on a guarded file. (Server entry:
> `store:bin/beagle-store-mcp`.)
...
The same graph, read instead of written: project a Beagle tree's AST into Beagle Store
...
```
*.bclj/.bjs/.bnix ──beagle-facts──▶ [subj "pred" obj] ──fold──▶ Beagle Store store ──Datalog──▶ callers / blast radius
...
```sh
# turn the stack on for a project dir — also writes <dir>/.store/corpus.facts
...
The public Beagle Store MCP data edge (`store:bin/beagle-store-mcp`) remains exactly `tell` /
...
- **`store:codegraph/` is an opt-in analysis surface.** Prefer the entry points
...
apps on the engine → store-modeling. Loop vocabulary:
