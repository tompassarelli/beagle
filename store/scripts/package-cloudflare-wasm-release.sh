...
# those bytes to the exact tagged Beagle Store source commit.
set -euo pipefail
export LC_ALL=C
umask 022
...
[[ "$(<"$artifact/READY")" == "beagle-store-native-build/v1 $artifact_identity" ]] ||
...
[[ "$(sed -n '1p' "$artifact/input.manifest")" == "beagle-store-native-build-input/v3" ]] ||
...
[[ "$(sed -n '1p' "$provenance")" == "beagle-store-native-build-provenance/v2" ]] ||
...
release_name="store-${version}-wasm32-wasm-embed"
...
$release_name/lib/
$release_name/lib/libstore.wasm
...
cmp -s "$artifact/lib/libstore.wasm" "$verify_root/lib/libstore.wasm" ||
...
wasm_sha256="$(sha256sum "$verify_root/lib/libstore.wasm" | awk '{print $1}')"
wasm_bytes="$(wc -c <"$verify_root/lib/libstore.wasm" | tr -d '[:space:]')"
...
store-cloudflare-wasm-release-receipt/v2
...
wasm-path $release_name/lib/libstore.wasm
