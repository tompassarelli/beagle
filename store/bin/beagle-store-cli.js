import { equivV as $$bc$equiv } from 'beagle/core.js';
...
function edn_json(text, term_mode) {
  return (() => { let index = 0; let output = ""; let quoted = false; let escaped = false; while (true) {
    if ((index >= text.length)) { return output; } else { const ch = text.substring(index, (index + 1)); if (quoted) { const _recur_0 = (index + 1); const _recur_1 = ("".concat(output, ch)); const _recur_2 = (escaped ? true : (!($$bc$equiv(ch, "\"")))); const _recur_3 = (escaped ? false : ($$bc$equiv(ch, "\\"))); index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } else { if (($$bc$equiv(ch, "\""))) { const _recur_0 = (index + 1); const _recur_1 = ("".concat(output, ch)); const _recur_2 = true; const _recur_3 = false; index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } else if (($$bc$equiv(ch, "("))) { const _recur_0 = (index + 1); const _recur_1 = ("".concat(output, "[")); const _recur_2 = false; const _recur_3 = false; index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } else if (($$bc$equiv(ch, ")"))) { const _recur_0 = (index + 1); const _recur_1 = ("".concat(output, "]")); const _recur_2 = false; const _recur_3 = false; index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } else if (($$bc$equiv(ch, ":"))) { const end = token_end(text, (index + 1)); const token = text.substring((index + 1), end); const value = (term_mode ? ("".concat("$kw:", token)) : token); const _recur_0 = end; const _recur_1 = ("".concat(output, JSON.stringify(value))); const _recur_2 = false; const _recur_3 = false; index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } else if ((term_mode && (!delimiter_p(ch)))) { const end = token_end(text, index); const token = text.substring(index, end); const _recur_0 = end; const _recur_1 = ("".concat(output, JSON.stringify(("".concat("$atom:", token))))); const _recur_2 = false; const _recur_3 = false; index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } else { const _recur_0 = (index + 1); const _recur_1 = ("".concat(output, ch)); const _recur_2 = false; const _recur_3 = false; index = _recur_0; output = _recur_1; quoted = _recur_2; escaped = _recur_3; continue; } } }
  } })();
}
...
function atom_term(bridge, token) {
  return ((($$bc$equiv(token, "true"))) ? bridge.booleanTerm(true) : (($$bc$equiv(token, "false"))) ? bridge.booleanTerm(false) : ((re_matches(/-?[0-9]+/, token) != null)) ? bridge.integerTerm(token) : ((re_matches(/-?(?:[0-9]+\.[0-9]*|[0-9]*\.[0-9]+)(?:[eE][+-]?[0-9]+)?/, token) != null)) ? bridge.floatTerm(token) : bridge.stringTerm(token));
}

function parsed_term(bridge, value) {
  return (((typeof value === 'string')) ? ((($$bc$equiv("$kw:", value.substring(0, 4)))) ? bridge.keywordTerm(value.substring(4)) : (($$bc$equiv("$atom:", value.substring(0, 6)))) ? atom_term(bridge, value.substring(6)) : bridge.stringTerm(value)) : (Array.isArray(value)) ? (($$bc$equiv(3, value.length)) ? bridge.tripleTerm(parsed_term(bridge, value[0]), parsed_term(bridge, value[1]), parsed_term(bridge, value[2])) : bridge.fail("term vector must contain exactly three values")) : (map_p(value)) ? (() => { const instant = host_get(value, "$kw:instant"); return ((Array.isArray(instant) && ($$bc$equiv(2, instant.length))) ? bridge.instantTerm(instant[0].substring(6), instant[1].substring(6)) : bridge.fail("term map must be {:instant [SECONDS NANOS]}")); })() : bridge.fail("unsupported human term"));
}
...
function render_row(bridge, row) {
  return ("".concat("[", ((..._xs) => "".concat(..._xs))(...map_indexed((index, value) => ("".concat((($$bc$equiv(index, 0)) ? "" : " "), render_term(bridge, value))), row)), "]"));
}
...
function scan_pattern(bridge, arguments$) {
  const pattern = js_obj();
  if ((!($$bc$equiv("_", arguments$[0])))) {
    aset(pattern, "t1", parse_term(bridge, arguments$[0]));
  }
  if ((!($$bc$equiv("_", arguments$[1])))) {
    aset(pattern, "t2", parse_term(bridge, arguments$[1]));
  }
  if ((!($$bc$equiv("_", arguments$[2])))) {
    aset(pattern, "t3", parse_term(bridge, arguments$[2]));
  }
  return pattern;
}
...
async function write_command(bridge, command, arguments$) {
  ...
  const request = js_obj("t1", parse_subject(bridge, arguments$[0]), "t2", parse_term(bridge, arguments$[1]), "t3", parse_term(bridge, arguments$[2]), "existing", existing, "expectedVersion", host_get(base, "servedVersion"));
  const response = await call(bridge, (asserting ? "assert" : "retract"), request);
  const result = host_get(response, "result")[0];
  const proposition = js_obj("subject", host_get(request, "t1"), "predicate", host_get(request, "t2"), "value", host_get(request, "t3"));
...
export async function run(bridge, raw_argv) {
...
const response = await call(bridge, "scan", js_obj("t1", subject));
...
export function classify_error(error) {
