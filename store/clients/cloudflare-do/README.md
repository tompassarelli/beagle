# `@tompassarelli/store-cloudflare-do`

Run the Beagle Store engine *inside* a Cloudflare Durable Object. The engine is
`lib/libstore.wasm`, built by `bin/beagle-store-native-build --host wasm-embed`; this
package is the host half of that seam — it answers all nine `store_host_v1`
imports out of DurableObjectStorage, so a Durable Object becomes a Beagle Store
...
```console
$ export BEAGLE_STORE_WASI_CC=/path/to/wasm32-unknown-wasi-clang
$ scripts/build-wasm.sh
build-wasm: lib/libstore.wasm <- <artifact-address> (<bytes> bytes, sha256 <hash>)
```

`scripts/build-wasm.sh` copies `lib/libstore.wasm` out of a content-addressed
`beagle-store-native-build` artifact and writes `lib/provenance.json` beside it, which
...
} from '@tompassarelli/store-cloudflare-do';
import { storeClient } from '@tompassarelli/beagle-store-rpc/core';
import { DurableObject, WorkerEntrypoint } from 'cloudflare:workers';
import storeModule from '../lib/libstore.wasm';

const SPACE = 'wiki.greywrought.com';

// Raw storage owner in a backend Worker. Never bind Beagle Store into the wiki Worker.
export class StoreLog extends DurableObject {
  #store;

  constructor(state, env) {
    super(state, env);
    this.#store = new StoreDurableObjectBase(
      state, env, storeModule, { spaceId: SPACE },
    );
  }
...
// The wiki Worker receives DATA_PLANE as a service binding, not env.BEAGLE_STORE.
export function client(DATA_PLANE) {
  return storeClient({
    space: SPACE,
    transport: storeDurableObjectTransport(DATA_PLANE),
  });
}
```
...
publication, restore throws `StoreBackupError` with code `restore-fenced` and an
...
transaction** — store boots through the image and replays the log tail, so the
pair must never be observed half-published.

Beagle Store's storage hooks must return before the guest call unwinds, and DO storage
...
- **Every exchange awaits the storage commit before it resolves.** A caller
  ...
  not yet durable.
- **Commits are atomic and serialised.** Beagle Store replays the log from byte zero, so
...
`StoreInstance.instantiate` compares the module's real import and export lists
...
`capacity/run-gate.sh` builds the current `libstore.wasm`, makes Wrangler emit
...
`BEAGLE_STORE_DO_WASM_ARTIFACT` and a dirty
...
workerd A, closes Beagle Store, and persists the Durable Object; the gate then proves
...
`BEAGLE_STORE_CF_CAPACITY_PLAN=paid` to check the Paid-plan 10 MiB limit. Both plans
...
additional conservative ceiling. The Worker also reports the exact Beagle Store engine
memory budget and guest-arena sizing it observed at runtime. The tracked profile
uses a 64 MiB engine budget and eight-page arena growth so Beagle Store compacts before
