...
//
// store_host_v1 over an async, Durable-Object-shaped storage API.
//
// store's storage hooks must return an i32 before the guest call unwinds and DO
...
function requestFail(message, code) {
  throw new StoreRequestError(message, code);
}
...
function entryAccepts(operation, entry) {
  ...
  return storeRpcEntry(operation) === entry;
}

export function storeDurableObjectTransport(stub) {
...
// store_wasm_host.c discriminates the two storage objects by context alone.
...
function backupFailure(code, message, options) {
  throw new StoreBackupError(code, message, options);
}
 ...
 */
export class DurableStoreStore {
  constructor(storage, options = {}) {
    const shared = {
...
/**
 * store's general allocator, living in a host-owned partition of guest memory.
 *
 * store_host_v1.allocate is not only for responses: the engine's own long-lived
...
class GuestArena {
  ...
  allocate(size) {
    ...
    if (header + need > this.end) {
      ...
      if (before < 0) return 0; // NULL -> store surfaces BEAGLE_STORE_OUT_OF_MEMORY
      this.grows += 1;
...
export class StoreInstance {
  /**
   * @param {WebAssembly.Module} module libstore.wasm, host=wasm-embed
   * @param {object} options
   *   store  - DurableStoreStore-shaped: load(which), commit(parts)
   *   nowMs  - () => epoch milliseconds; defaults to Date.now
   *   arena  - GuestArena options
   *   memoryBudgetBytes - store_open_options_v1.memory_budget_bytes; 0 = default
   */
  static async instantiate(module, options = {}) {
    const instance = new StoreInstance(options);
  ...
  #imports() {
    ...
    const wasi_snapshot_preview1 = {
      // The Beagle shim's monotonic clock and getenv have no store_host_v1
    ...
    return { store_host_v1, wasi_snapshot_preview1 };
  }
  ...
  alloc(size) {
    const pointer = this.exports.store_wasm_alloc(size);
    if (!pointer) throw new Error(`store_wasm_alloc(${size}) returned NULL`);
    this.#bytes().fill(0, pointer, pointer + size);
    return pointer;
  }

  free(pointer) {
    this.exports.store_wasm_free(pointer);
  }
  ...
  open(spaceId, logLabel = "in-memory") {
    return this.#serialise(async () => {
      ...
      const view = this.#view();
      view.setUint32(options + 0, BEAGLE_STORE_ABI_VERSION, true);
      ...
      const status = this.exports.store_open(options, databaseOut, error);
  ...
  close() {
    return this.#serialise(async () => {
      const error = this.alloc(ERROR_SIZE);
      const status = this.exports.store_close(this.database, error);
      const message = this.readCString(
        error + ERROR_MESSAGE_OFFSET,
        ERROR_SIZE - ERROR_MESSAGE_OFFSET,
      );
   ...
   * replaced, even though StoreDurableObjectBase has dropped its own reference.
   */
  fence(error) {
    const run = async () => {
  ...
  async #callNow(entry, packet) {
    ...
    this.exports.store_buffer_release(buffer);
  ...
  #assertUsable() {
    ...
    if (this.closed) throw new Error("this store instance is closed");
  }
  ...
  async #commit() {
    ...
    try {
      ...
      this.poisoned = new StoreStorageError(error);
      throw this.poisoned;
    }
    this.commits += 1;
  }
 ...
 */
export class StoreDurableObjectBase {
  constructor(state, env, module, options = {}) {
    if (typeof options.spaceId !== "string" || !options.spaceId) {
      throw new TypeError("Beagle Store Durable Object spaceId must be a nonempty string");
    }
    if (textEncoder.encode(options.spaceId).length > BEAGLE_STORE_DO_MAX_NAMED_SPACE_BYTES) {
      throw new TypeError("Beagle Store Durable Object spaceId exceeds 1024 UTF-8 bytes");
    }
  ...
  async #boot() {
    const store = new DurableStoreStore(this.state.storage, this.storeOptions);
    const instance = await StoreInstance.instantiate(this.module, {
    ...
    if (opened.status !== 0) {
      throw new Error(`store_open failed: ${opened.status} ${opened.message}`);
    }
    this.store = store;
    return instance;
  }
  ...
  async #bootForRestore() {
    const durableStore = new DurableStoreStore(
      this.state.storage,
      this.storeOptions,
    );
    const readOnlyStore = {
  ...
  async exchange(packet, options = {}) {
    ...
    if (!result.released) {
      throw new StoreExchangeError(result.status, "guest response buffer was not released");
    }
  ...
  async #preflightStoreLog(bytes, expectedServedVersion) {
    ...
    let instance = null;
    try {
      instance = await StoreInstance.instantiate(this.module, {
        ...this.instanceOptions,
      ...
      if (error instanceof StoreBackupError) throw error;
      backupFailure(
        "engine",
        `cannot replay the backup through store: ${error.message}`,
        { cause: error },
      );
    } finally {
      if (instance) {
        await instance.fence(
          new StoreBackupError(
            "administrative-fence",
            "this Beagle Store instance belonged to restore preflight",
          ),
        );
      }
    }
 ...
 */
export function storeDataPlaneEntrypoint(namespace, spaceId) {
  if (!namespace || (typeof namespace !== "object"
      && typeof namespace !== "function")) {
    throw new TypeError("Beagle Store data plane requires a Durable Object namespace");
  }
  if (typeof spaceId !== "string" || !spaceId) {
    throw new TypeError("Beagle Store data plane spaceId must be a nonempty string");
  }
  if (textEncoder.encode(spaceId).length > BEAGLE_STORE_DO_MAX_NAMED_SPACE_BYTES) {
    throw new TypeError("Beagle Store data plane spaceId exceeds 1024 UTF-8 bytes");
  }
 ...
 */
export function storeAdminEntrypoint(namespace, spaceId) {
  if (!namespace || (typeof namespace !== "object"
      && typeof namespace !== "function")) {
    throw new TypeError("Beagle Store administration requires a Durable Object namespace");
  }
  if (typeof spaceId !== "string" || !spaceId) {
    throw new TypeError("Beagle Store administration spaceId must be a nonempty string");
  }
  if (textEncoder.encode(spaceId).length > BEAGLE_STORE_DO_MAX_NAMED_SPACE_BYTES) {
    throw new TypeError("Beagle Store administration spaceId exceeds 1024 UTF-8 bytes");
  }
  const object = () => namespace.getByName(spaceId);
  return Object.freeze({
    exportStoreLog() {
