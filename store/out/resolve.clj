...
;;
;;   bb -cp ~/code/beagle/store/out store/out/resolve.clj resolve <edn>...
;;   bb -cp ~/code/beagle/store/out store/out/resolve.clj rename <old> <new> <target> <edn>...
;;   bb -cp ~/code/beagle/store/out store/out/resolve.clj delete <name> <target> <edn>...
;; ============================================================================
(ns resolve
  (:require [clojure.edn :as edn] [clojure.string :as str]
            [store.datalog :as d] [cheshire.core :as json]   ; datalog+json: the `callgraph` mode
            [store.rotation :as rot] [store.txn :as txn] [store.types :as t]
...
;; the server falls back to a whole-corpus re-resolve (sound; dormant in Store, which
;; has zero defmacro).
...
;; legitimately be the SHORT module name ("schema" for module "store.schema"), which the
...
;; trailing dotted-suffix of it ("schema" ⊴ "store.schema" ✓; "cheshire.generate" ⋬
...
;; projecting render EDN for ONLY that module. No src/store/*.bclj is ever read:
...
;; from any PUBLIC binding. Two strata via store.datalog/run-strata:
