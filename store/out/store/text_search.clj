(ns store.text-search
  (:require [clojure.string :as str]
            [store.text-index :as text-index]
            [store.types :as t]))
...
(defn word-tokens [^String value]
  ...
  (loop [index 0
   tokens []]
  (if (>= index (count parts)) tokens (let [^String token (nth parts index)]
...
(defn- token-rows-weight [rows]
  (reduce (fn [total tokens] (let [token-weight (loop [remaining tokens
   subtotal 0]
  (if (empty? remaining) subtotal (recur (vec (rest remaining)) (+ subtotal 42 (* 4 (count (first remaining)))))))]
...
(defn- ^SearchBuildState build-state [rows]
  (loop [handle 0
   ^SearchBuildState state (->SearchBuildState [] [] {} {})]
...
(defn- ^SearchAnalyzersResult analyzers-of-result! [^TextSearchSource source]
  ...
  (if (nil? current) (let [^SearchBuildState state (build-state (textsearchsource-rows source))
   ...
   ^SearchAnalyzers analyzers (->SearchAnalyzers token-rows normalized-rows stem-postings trigram-postings weight)
...
(defn- ^SearchAnalyzers analyzers-of! [^TextSearchSource source]
  (let [^SearchAnalyzersResult result (analyzers-of-result! source)]
...
(defn- ^String string-needle [^String relation needle]
  (let [^StringNeedleResult result (string-needle-result relation needle)]
...
(defn- ^WordNeedleResult word-needle-result [^String relation needle]
  (let [^StringNeedleResult string-result (string-needle-result relation needle)]
...
(defn- word-needle [^String relation needle]
  (let [^WordNeedleResult result (word-needle-result relation needle)]
...
(defn- ^StringNeedleResult substring-needle-result [^String relation needle]
  (let [^StringNeedleResult string-result (string-needle-result relation needle)]
  (if (not (stringneedleresult-ok string-result)) string-result (let [^String value (lower (stringneedleresult-value string-result))]
...
(defn- ^String substring-needle [^String relation needle]
  (let [^StringNeedleResult result (substring-needle-result relation needle)]
  (if (stringneedleresult-ok result) (stringneedleresult-value result) (text-index/raise-text-error (stringneedleresult-error result)))))
...
(defn ^Boolean substring-needle-valid? [needle]
  (if (string? needle) (let [^String text needle]
...
(defn ^TextSearchHandlesResult phrase-handles-result! [^TextSearchSource source needle]
  (let [^WordNeedleResult needle-result (word-needle-result "text-phrase" needle)]
  ...
  (if (not (text-index/handles-result-ok? indexed-result)) (handles-error (text-index/handles-result-error indexed-result)) (let [^SearchAnalyzersResult analyzers-result (analyzers-of-result! source)]
...
(defn phrase-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (phrase-handles-result! source needle)]
...
(defn ^TextSearchHandlesResult phrase-scan-handles-result! [^TextSearchSource source needle]
  (let [^WordNeedleResult needle-result (word-needle-result "text-phrase" needle)]
  (if (not (wordneedleresult-ok needle-result)) (handles-error (wordneedleresult-error needle-result)) (let [^SearchAnalyzersResult analyzers-result (analyzers-of-result! source)]
  (if (not (searchanalyzersresult-ok analyzers-result)) (handles-error (searchanalyzersresult-error analyzers-result)) (let [tokens (wordneedleresult-tokens needle-result)
   rows (searchanalyzers-token-rows (searchanalyzersresult-analyzers analyzers-result))]
...
(defn phrase-scan-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (phrase-scan-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))

(defn ^TextSearchHandlesResult substring-handles-result! [^TextSearchSource source needle]
  (let [^StringNeedleResult needle-result (substring-needle-result "text-substring" needle)]
  (if (not (stringneedleresult-ok needle-result)) (handles-error (stringneedleresult-error needle-result)) (let [^String value (stringneedleresult-value needle-result)
   grams (vec (trigrams value))
   ^SearchAnalyzersResult analyzers-result (analyzers-of-result! source)]
  (if (not (searchanalyzersresult-ok analyzers-result)) (handles-error (searchanalyzersresult-error analyzers-result)) (let [^SearchAnalyzers analyzers (searchanalyzersresult-analyzers analyzers-result)
...
(defn substring-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (substring-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))

(defn ^TextSearchHandlesResult substring-scan-handles-result! [^TextSearchSource source needle]
  (let [^StringNeedleResult needle-result (substring-needle-result "text-substring" needle)]
  (if (not (stringneedleresult-ok needle-result)) (handles-error (stringneedleresult-error needle-result)) (let [^SearchAnalyzersResult analyzers-result (analyzers-of-result! source)]
  (if (not (searchanalyzersresult-ok analyzers-result)) (handles-error (searchanalyzersresult-error analyzers-result)) (let [^String value (stringneedleresult-value needle-result)
...
(defn substring-scan-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (substring-scan-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))

(defn ^TextSearchHandlesResult stem-handles-result! [^TextSearchSource source needle]
  (let [^WordNeedleResult needle-result (word-needle-result "text-stem" needle)]
...
(defn stem-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (stem-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))
...
(defn stem-scan-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (stem-scan-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))
...
(defn- ^TextSearchScoreResult score-at-result! [^TextSearchSource source handle needle]
  (let [^StringNeedleResult needle-result (substring-needle-result "text-search" needle)]
  (if (not (stringneedleresult-ok needle-result)) (->TextSearchScoreResult false 0 (stringneedleresult-error needle-result)) (let [^SearchAnalyzersResult analyzers-result (analyzers-of-result! source)]
  (if (not (searchanalyzersresult-ok analyzers-result)) (->TextSearchScoreResult false 0 (searchanalyzersresult-error analyzers-result)) (let [^String value (stringneedleresult-value needle-result)
   query-tokens (word-tokens value)
   query-stems (vec (stems query-tokens))
   ^SearchAnalyzers analyzers (searchanalyzersresult-analyzers analyzers-result)
   ...
   ^String normalized (nth (searchanalyzers-normalized-rows analyzers) handle)
   ^Boolean substring-match (str/includes? normalized value)
   ^Boolean stem-covered (and (not (empty? query-stems)) (every? (fn [^String key] (contains? document-stems key)) query-stems))
   ^Boolean phrase-match (and (not (empty? query-tokens)) (sequence-contains? document-tokens query-tokens))
   score (if (not (or substring-match stem-covered)) 0 (let [token-score (reduce (fn [total ^String token] (let [match-score (cond
  (contains? document-token-set token) 100
  (contains? document-stems (english-stem token)) 60
  :else 0)
   match-count (min 3 (count-token document-tokens token))]
  (+ total match-score (* 5 match-count)))) 0 query-tokens)
   phrase-score (if phrase-match 250 0)
   substring-score (if substring-match 80 0)
   exact-score (if (= normalized value) 400 0)
   raw (+ token-score phrase-score substring-score exact-score)
...
(defn- score-at! [^TextSearchSource source handle needle]
  (let [^TextSearchScoreResult result (score-at-result! source handle needle)]
...
(defn- ^TextSearchHandlesResult matching-score-handles-result! [^TextSearchSource source needle candidates]
  ...
  (if (>= position (count candidates)) (handles-ok matches) (let [handle (nth candidates position)
   ^TextSearchScoreResult result (score-at-result! source handle needle)]
...
(defn ranked-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (ranked-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))
...
(defn ranked-scan-handles! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult result (ranked-scan-handles-result! source needle)]
  (if (handles-result-ok? result) (handles-result-handles result) (text-index/raise-text-error (handles-result-error result)))))
...
(defn- ^TextSearchRowsResult ranked-rows-for-handles-result! [^TextSearchSource source needle handles]
  ...
  (if (>= position (count handles)) (rows-ok result-rows) (let [handle (nth handles position)
   ^TextSearchScoreResult score-result (score-at-result! source handle needle)]
...
(defn- ranked-rows-for-handles! [^TextSearchSource source needle handles]
  (let [^TextSearchRowsResult result (ranked-rows-for-handles-result! source needle handles)]
...
(defn exact-indexed-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (exact-indexed-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn exact-scan-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (exact-scan-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))
...
(defn ^TextSearchRowsResult ranked-indexed-rows-result! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult handles-result (ranked-handles-result! source needle)]
...
(defn ^TextSearchRowsResult ranked-scan-rows-result! [^TextSearchSource source needle]
  (let [^TextSearchHandlesResult handles-result (ranked-scan-handles-result! source needle)]
  (if (handles-result-ok? handles-result) (ranked-rows-for-handles-result! source needle (handles-result-handles handles-result)) (rows-error (handles-result-error handles-result)))))

(defn phrase-indexed-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (phrase-indexed-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn phrase-scan-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (phrase-scan-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn substring-indexed-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (substring-indexed-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn substring-scan-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (substring-scan-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn stem-indexed-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (stem-indexed-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn stem-scan-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (stem-scan-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn ranked-indexed-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (ranked-indexed-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))

(defn ranked-scan-rows! [^TextSearchSource source needle]
  (let [^TextSearchRowsResult result (ranked-scan-rows-result! source needle)]
  (if (rows-result-ok? result) (rows-result-rows result) (text-index/raise-text-error (rows-result-error result)))))
