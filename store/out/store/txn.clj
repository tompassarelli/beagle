...
(defn mint-coordinate [transaction ordinal]
  (if (and (t/transaction-coordinate? transaction) (>= ordinal 0)) (t/triple transaction mint-ordinal ordinal) (throw (ex-info "store: a mint coordinate requires a transaction coordinate and non-negative ordinal" {:type :invalid-mint-coordinate}))))
...
(defn commit! [ctx builder]
  ...
  (if (not (= (t/triple-t1 pinned) (store/space-id ctx))) (store/->CommitRejected :transaction-space-mismatch "store: transaction belongs to a different space") (store/commit-boundary! ctx (t/triple-t3 pinned) (builder-operations current) (store/commit-metadata "store.txn/v1" "store/CommitOperationV1" "store-schema-v1")))))
