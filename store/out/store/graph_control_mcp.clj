(ns store.graph-control-mcp
  ...
  (:require [cheshire.core :as json]
            ...
            [store.program-inspection :as program]
            [store.projection-lifecycle :as lifecycle]
            [store.rt :as rt]
            [store.types :as t])
...
(def ^:private preflight-contract "store.graph-control-preflight/v1")
(def ^:private server-name "store-graph-control")
...
(defn- port! []
  (let [raw (required-env! "BEAGLE_STORE_CODE_PORT")]
    (try
      (let [value (Long/parseLong raw)]
        (when-not (<= 1 value 65535)
          (fail! :invalid-sealed-binding
                 "BEAGLE_STORE_CODE_PORT is outside the TCP port range" {:port raw}))
        value)
      (catch NumberFormatException _
        (fail! :invalid-sealed-binding
               "BEAGLE_STORE_CODE_PORT must be a decimal TCP port" {:port raw})))))
...
(defn- beagle-facts-command! [beagle]
  (let [command (io/file (.getParentFile (io/file beagle)) "beagle-facts")]
    (when-not (and (.isFile command) (.canExecute command))
      (fail! :missing-sealed-binding
             "BEAGLE_STORE_BEAGLE must have an executable beagle-facts sibling"
             {:beagle beagle}))
...
(defn preflight!
  ...
  []
  (let [checkout-root (canonical-path! (required-env! "BEAGLE_STORE_CHECKOUT_ROOT")
                                       "checkout root")
        source-root (canonical-path! (required-env! "BEAGLE_STORE_SOURCE_ROOT")
                                     "source root")
        code-log (canonical-path! (required-env! "BEAGLE_STORE_CODE_LOG") "code log")
        port (port!)
        ...
        rendered (when (:accepted check)
                   (code-reader/render-module!
                    (required-env! "BEAGLE_STORE_BEAGLE") snapshot))
        version-after
...
(defn- form! [value]
  ...
  (let [annotation-token (symbol (str "__store_annotation_" (gensym)))
...
(defn- read-program!
  ...
  (when-not program-corpus
    (fail! :program-corpus-unavailable
           "this checkout has no resolved program corpus at .store/corpus.facts"
           {:tool tool}))
...
(defn- program-corpus-path
  "Locate beagle-store-code-on's resolved reference corpus for this checkout.
  ...
  [checkout-root]
  (let [file (io/file checkout-root ".store" "corpus.facts")]
    (when (.isFile file) (.getCanonicalPath file))))

(defn- service-context! []
  (let [preflight (preflight!)
        code-log (required-env! "BEAGLE_STORE_CODE_LOG")
        checkout-root (canonical-path! (required-env! "BEAGLE_STORE_CHECKOUT_ROOT")
                                       "checkout root")]
    {:preflight preflight
     :checkout-root checkout-root
     :port (port!)
     :space (space-id! code-log)
     :beagle (required-env! "BEAGLE_STORE_BEAGLE")
     :program-corpus (program-corpus-path checkout-root)
     :program-facts-command
     (beagle-facts-command! (required-env! "BEAGLE_STORE_BEAGLE"))}))

(defn- serve! []
  (let [context (service-context!)]
    (binding [*out* *err*]
      (println "store-graph-control: sealed preflight PASS; ready on stdio"))
    (loop []
