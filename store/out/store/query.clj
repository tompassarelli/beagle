...
(defn- ^QueryResult abort-result [problem]
  ...
  (if (= :store-query-abort (:type data)) (failure-result [(query-error (:code data) (ex-message problem))]) (throw problem))))
...
(defn- ^QueryResult execution-result-or-raise! [^QueryExecutionResult execution]
  ...
  (cond
  (some? result) result
  (some? error-value) (if (= :store-query-abort (d/query-evaluation-error-type error-value)) (failure-result [(query-error (d/query-evaluation-error-code error-value) (d/query-evaluation-error-message error-value))]) (d/raise-query-evaluation-error! error-value))
...
(def ^String page-cursor-prefix "store-query-term-page-v1.")
...
(defn- ^QueryPage abort-page [problem]
  (let [data (ex-data problem)]
  (if (= :store-query-abort (:type data)) (failure-page [(query-error (:code data) (ex-message problem))]) (throw problem))))

(defn- ^QueryPage evaluation-error-page-or-raise! [error-value]
  (if (= :store-query-abort (d/query-evaluation-error-type error-value)) (failure-page [(query-error (d/query-evaluation-error-code error-value) (d/query-evaluation-error-message error-value))]) (d/raise-query-evaluation-error! error-value)))
