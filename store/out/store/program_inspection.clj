(ns store.program-inspection
  "Snapshot-pinned semantic reads over beagle-store-code-on's resolved corpus."
...
(def ^:private view-schema "store.program-view/v1")
...
(defn- atomic-write! [file bytes]
  ...
  (let [path (.toPath (io/file file))
        temp (Files/createTempFile (.getParent path) ".store-program-view-" ".tmp"
