(ns store.authority
  (:require [clojure.string :as str]
            [store.authority-json :as authority-json]))
...
(defn validate-tool-catalog! [value]
  ...
  (do
  (ensure-authority! (= "store.graph-edit-tools/v1" (clean-text! (get m "catalogVersion") "tools.catalogVersion")) "catalog-version" "tools.catalogVersion" "unsupported catalogVersion")
...
(defn normalize-module-manifest! [source-root-relative-to-checkout graph-version entries-value ast-module-ids-value]
  (let [source-root (clean-text! source-root-relative-to-checkout "manifest.sourceRootRelativeToCheckout")
   ...
   mapping-core {"manifestVersion" "store.module-manifest/v1" "sourceRootRelativeToCheckout" source-root "entries" ordered}
  ...
  {"manifestVersion" "store.module-manifest/v1" "sourceRootRelativeToCheckout" source-root "graphVersion" version "mappingDigest" (digest-json-no-number-no-null! mapping-core) "snapshotDigest" (digest-json-no-number-no-null! snapshot-core) "entries" ordered})))
...
(defn validate-normalized-module-manifest! [value]
  ...
  (do
  (ensure-authority! (= "store.module-manifest/v1" (clean-text! (get m "manifestVersion") "manifest.manifestVersion")) "manifest-version" "manifest.manifestVersion" "unsupported manifestVersion")
...
(defn validate-authority-binding! [value]
  ...
  (do
  (ensure-authority! (= "store.graph-edit-authority-binding/v1" (clean-text! (get m "bindingVersion") "binding.bindingVersion")) "binding-version" "binding.bindingVersion" "unsupported bindingVersion")
...
(defn validate-authority-descriptor! [value]
  ...
  (do
  (ensure-authority! (= "store.graph-edit-authority/v1" (clean-text! (get m "descriptorVersion") "descriptor.descriptorVersion")) "descriptor-version" "descriptor.descriptorVersion" "unsupported descriptorVersion")
...
(defn- validate-receipt-envelope! [m]
  ...
  (do
  (ensure-authority! (= "store.graph-edit-phase-receipt/v1" (clean-text! (get m "receiptVersion") "receipt.receiptVersion")) "receipt-version" "receipt.receiptVersion" "unsupported receiptVersion")
