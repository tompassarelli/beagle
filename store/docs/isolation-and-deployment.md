...
The server consumes only a linked executable promoted behind the native artifact's READY marker. Raw Beagle projection output and compiled Clojure under `out/` are not executable server runtime inputs. QBE remains the direct-native cross-check.
