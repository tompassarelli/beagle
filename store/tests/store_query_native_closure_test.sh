...
beagle="${BEAGLE_STORE_BEAGLE:-$(command -v beagle || true)}"
scratch="$(mktemp -d)"
cleanup() { rm -rf "${scratch:?}"; }
trap cleanup EXIT INT TERM

fail() {
  echo "store query native closure: FAIL: $*" >&2
  exit 1
}
...
(
  ...
  "$beagle" build --materializer c17 --out "$scratch/artifact" --abi wasm32 \
    --entry store.query/plan-text-attribute-scope \
    --entry store.query/term-compare \
    --entry store.query/ordered-plan-rows \
    -- "${sources[@]}"
...
printf 'store query native closure: PASS report=%s\n' "$report"
