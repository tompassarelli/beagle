...
mkdir -p "$source_root" "$checkout/.store" "$checkout/.codex" "$evil/bin" "$evil/home/code/beagle"
printf '%s\n' '{:tx 1 :op "assert" :l "@empty#root" :p "file" :r "empty.bclj"}' >"$code_log"
printf '%s\n' '[mcp_servers.beagle-store]' 'command = "/definitely/hostile"' >"$checkout/.codex/config.toml"
...
common_hostile=(
  ...
  BEAGLE_STORE_GRAPH_EDIT_SEALED_STORE="$evil/store"
