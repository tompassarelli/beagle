...
;; mcp__store__set-body tool commits through — over a throwaway daemon booted on a
...
;; (BEAGLE_HOME / BEAGLE_STORE_BEAGLE); SKIPs cleanly if Beagle can't be resolved.
;;
;;   BEAGLE_HOME=$HOME/code/beagle/main \
;;     clojure -M tests/code_editpath_defect_test.clj ; echo EXIT=$?
;; ============================================================================
(require '[store.store :as c] '[store.schema :as s]
...
(when-not (.canExecute (io/file beagle-bin))
  (println "SKIP — no Beagle CLI (set BEAGLE_STORE_BEAGLE / BEAGLE_HOME)") (System/exit 0))
...
(def env (assoc (into {} (System/getenv))
                "BEAGLE_HOME" beagle-home "BEAGLE_STORE_BEAGLE" beagle-bin))
(let [r (sh/sh "bb" "-cp" "out" "bin/beagle-store-ingest-code"
...
;; set-body through the warm :edit-min wire op (the mcp__store__set-body path).
