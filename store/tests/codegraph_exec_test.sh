...
# runs rename, roundtrip_store and supersession_check against the in-tree fixtures
...
for ns in callgraph codegraph rename rep-jurisdiction roundtrip-store supersession-check; do
...
# roundtrip_store: source -> facts -> a real store -> facts, verified back to source.
if timeout 240 bb -cp out -m roundtrip-store "$trap_edn" > "$scratch/rt.edn" 2>"$scratch/rt.err" &&
  ...
  pass "roundtrip_store: trap.bjs round-trips through a Beagle Store store (--verify)"
else
  fail "roundtrip_store: $(tail -3 "$scratch/rt.err" "$scratch/verify.out" | tr '\n' ' ')"
fi
