...
async function startServer(port, log, space) {
  ...
  const child = Bun.spawn([
    resolve(root, 'bin/beagle-store-server'),
    'serve',
    String(port),
    log,
    space,
  ...
  const consume = async stream => {
    ...
    try {
      for await (const chunk of stream) {
        ...
        if (!ready && output.includes('Beagle Store server listening')) {
          ready = true;
          clearTimeout(timer);
          resolveReady();
        }
...
async function exerciseClient(store) {
  ...
  await check('expected-version assert preserves recursive Terms exactly', async () => {
    const response = await store.assert('@doc-a', keywordTerm('title'), 'Running with Wolves', {
      expectedVersion: 0n,
    });
    ...
    const nested = await store.assert(
  ...
  await check('batch is atomic and stale expected-version writes fail typed', async () => {
    ...
    await assert.rejects(
      store.assert('@stale', keywordTerm('title'), 'must not land', { expectedVersion: 0n }),
      error => error instanceof StoreRpcError && error.code === 'rpc/conflict'
        && error.retryable === true && error.servedVersion === 3n,
    );
    assert.equal((await store.version()).servedVersion, 3n);
  });
  ...
  await check('scan pagination pins the served snapshot and carries its exact cursor', async () => {
    ...
    const first = await store.scan(pattern, { page: { limit: 2 } });
    ...
    const second = await store.scan(pattern, {
  ...
  await check('recursive ask supports current and historical structured queries', async () => {
    const current = await store.query(titleQuery, { timeoutMs: 5000 });
    assert.equal(current.result.length, 3);
    assert.equal(current.servedVersion, 3n);

    const historical = await store.query(titleQuery, { asOf: firstVersion, timeoutMs: 5000 });
  ...
  await check('structured query order and top-K are evaluated by Beagle Store', async () => {
    const ranked = await store.query({
      ...titleQuery,
      orderBy: [
  ...
  await check('occurrence replay and validate retain typed payloads', async () => {
    const occurrences = await store.occurrences({ page: { limit: 16 } });
  ...
  await check('lease acquire, check, renew, release, and stale check are exact', async () => {
    const acquired = await store.leaseAcquire('builder', 'plangrep', 30000);
    assert(acquired.result.fence);
    const checked = await store.leaseCheck(acquired.result.fence);
    assert.equal(checked.result.valid, true);

    const renewed = await store.leaseRenew(acquired.result.fence, 30000);
    const released = await store.leaseRelease(renewed.result.fence);
    assert.equal(released.result.released, true);
    const stale = await store.leaseCheck(renewed.result.fence);
    assert.equal(stale.result.valid, false);
  });

  await check('retract completes the closed thirteen-operation client surface', async () => {
    const response = await store.retract('@doc-c', keywordTerm('title'), 'Other Notes');
    assert.equal(response.result[0].stateChanged, true);
    assertOccurrenceCoordinate(response.result[0].occurrence, response.servedVersion, 0n);
    const scan = await store.scan({ t1: '@doc-c' });
    assert.equal(scan.result.length, 0);

    const beforeNoMatch = (await store.version()).servedVersion;
    const noMatch = await store.retract('@missing', keywordTerm('title'), 'Missing');
  ...
  await check('schema writes compose identity guards and occurrence-correct replacement on the live server', async () => {
    ...
    await store.assert(subject, title, firstTitle, { expectedVersion: created.servedVersion });
    ...
    const current = await store.scan({ t1: subject, t2: title });
    ...
    const beforePublish = await store.version();
    ...
    assert.deepEqual(
      (await store.scan({ t1: revisionSubject, t2: revisionStatus })).result,
      [tripleTerm(revisionSubject, revisionStatus, canonical)],
    );
    assert.deepEqual(
      (await store.scan({ t1: subject, t2: canonicalRevision })).result,
    ...
    assert.deepEqual(
      new Set((await store.scan({ t1: subject, t2: linksTo })).result.map(JSON.stringify)),
      new Set([
    ...
    const beforeRejectedLinks = await store.version();
    await assert.rejects(
    ...
    assert.equal((await store.version()).servedVersion, beforeRejectedLinks.servedVersion);
    assert.deepEqual(
      (await store.scan({ t1: subject, t2: title })).result,
      [tripleTerm(subject, title, canonicalTitle)],
    );

    const beforeRejectedPublish = await store.version();
    await assert.rejects(
    ...
    assert.equal((await store.version()).servedVersion, beforeRejectedPublish.servedVersion);
    assert.deepEqual(
      (await store.scan({ t1: revisionSubject, t2: revisionStatus })).result,
      [tripleTerm(revisionSubject, revisionStatus, canonical)],
    );
  ...
  await check('schema mixed transactions create planned graphs and update existing owners atomically', async () => {
    ...
    await store.assert(page, temporary, temporaryValue);
    ...
    assert.deepEqual(
      (await store.scan({ t1: nodeA, t2: graphNext })).result,
      [tripleTerm(nodeA, graphNext, nodeB)],
    );
    assert.deepEqual(
      (await store.scan({ t1: nodeB, t2: graphNext })).result,
      [tripleTerm(nodeB, graphNext, nodeC)],
    );
    assert.deepEqual(
      (await store.scan({ t1: nodeC, t2: graphNext })).result,
      [tripleTerm(nodeC, graphNext, nodeA)],
    );
    assert.deepEqual((await store.scan({ t1: page, t2: temporary })).result, []);

    const staleSubject = stringTerm('@stale-create');
    const beforeStale = await store.version();
    await assert.rejects(
      schema.transactUnique({
        creates: [{
    ...
    assert.equal((await store.version()).servedVersion, beforeStale.servedVersion);
    assert.deepEqual((await store.scan({ t1: staleSubject })).result, []);

    const beforeExistingReservation = await store.version();
    await assert.rejects(
      schema.transactUnique({
        creates: [{
          subject: reservation,
    ...
    assert.equal(
      (await store.version()).servedVersion,
