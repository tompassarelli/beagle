;; store_mcp.clj — the public AI-facing data edge of a Beagle Store instance.
...
(def instructions
  (str
   "Beagle Store is a recursive Triple engine: every proposition has t1/t2/t3. "
...
(defn- native-error-result [response]
  (when-let [error (store.rt/native-error response)]
    {:isError true
...
(defn- mcp-subject! [value]
  (if (and (string? value) (not (str/starts-with? value "@")))
    (str "@" value)
    (store.rt/lower-term! value)))

(defn- native-version! []
  (let [response (store.rt/native-call!
                  (store.rt/server-port) :rpc/version rpc-wire/rpc-unit)]
    (store.rt/require-native-success! response)
  ...
  (try
    (let [operation (if (= name "tell") :rpc/assert :rpc/retract)
          ...
          (terms/triple subject
                        (store.rt/lower-term! (:predicate arguments))
                        (store.rt/lower-term! (:object arguments)))]
      (loop [remaining 5]
        (let [base (native-version!)
              response
              (store.rt/native-call!
               (store.rt/server-port) (store.rt/rpc-space-id) operation
               (rpc-wire/rpc-write! proposition rpc-wire/rpc-subject-any nil)
               base nil nil)]
          (cond
            (and (= :rpc/conflict (store.rt/native-error-code response))
                 (pos? remaining))
            (recur (dec remaining))

            (store.rt/native-error response)
            ...
            {:text
             (json/generate-string
              (mapv
               ...
               (store.rt/rpc-list-values! values)))})))
    (catch Throwable error
      {:isError true :text (or (.getMessage error) (str (class error)))})))
...
(defn- native-mcp-query [arguments]
  (try
    (let [response
          (store.rt/native-call!
           (store.rt/server-port) :rpc/query
          ...
          (let [[valid violations]
            ...
            {:text
             (json/generate-string
              {:valid valid
               :violations
               (mapv term-json (store.rt/rpc-list-values! violations))})})))
    (catch Throwable error
      {:isError true :text (or (.getMessage error) (str (class error)))})))
  ...
  (if (> (swap! live-queries inc) max-live-queries)
    ...
    (let [result (promise)
          worker (doto (Thread.
                   ...
                   (.setName "beagle-store-mcp-query")
                   (.start))
  ...
  (let [has-id (contains? req :id)
    ...
    (cond
      ...
      (reply id {:protocolVersion "2024-11-05"
                 :capabilities {:tools {}}
                 :serverInfo {:name "store" :version "0.1"}
...
(log! "beagle-store-mcp: ready on stdio (public data catalog: tell/retract/show/ask/validate)")
(loop []
