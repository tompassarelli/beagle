...
;; It is selected only by pointing the production BEAGLE_STORE_EDIT_VERIFIER launch seam
...
(def mode (or (System/getenv "BEAGLE_STORE_EDIT_VERIFIER_FIXTURE_MODE") "delegate"))
(def required-source (System/getenv "BEAGLE_STORE_EDIT_VERIFIER_REQUIRE_SOURCE"))
...
(def toolchain-closure-digest
  (sha256-hex ["beagle-store-edit-verifier-fixture" 1]))
...
(when-let [path (System/getenv "BEAGLE_STORE_EDIT_VERIFIER_COUNT_FILE")]
  (spit path (str (:candidate request) "\n") :append true))

(when-let [raw (System/getenv "BEAGLE_STORE_EDIT_VERIFIER_SLEEP_MS")]
...
(case mode
  "delegate"
  (let [adapter (System/getenv "BEAGLE_STORE_EDIT_VERIFIER_FIXTURE_DELEGATE")]
    (when (str/blank? adapter)
      (binding [*out* *err*]
        (println "BEAGLE_STORE_EDIT_VERIFIER_FIXTURE_DELEGATE is required"))
      (System/exit 2))
    (let [result
          (process/shell
