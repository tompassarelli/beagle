...
store_up_source="${BEAGLE_STORE_UP_UNDER_TEST:-$repo_root/bin/beagle-store-up}"
...
fail() {
  echo "beagle-store-up readiness: FAIL: $*" >&2
  exit 1
}
...
make_case() {
  ...
  cp "$store_up_source" "$case_dir/bin/beagle-store-up"
  chmod +x "$case_dir/bin/beagle-store-up"
...
[[ "$BEAGLE_STORE_SPACE_ID" == "$(cat "$TEST_STATE/served-space")" ]] || exit 42
if [[ "${TEST_MODE:-}" == "slow-start" ]]; then
  "$TEST_SLEEP" 0.5
fi
STUB

  cat >"$case_dir/bin/beagle-store-server" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail
printf '%s|%s\n' "$1" "$2" >>"$TEST_STATE/server.calls"
if [[ "${TEST_MODE:-}" != "unavailable" ]]; then
  printf '%s\n' "$BEAGLE_STORE_SPACE_ID" >"$TEST_STATE/served-space"
fi
STUB

  cat >"$case_dir/bin/beagle" <<'STUB'
#!/usr/bin/env bash
echo "heavyweight store CLI invoked: $*" >&2
exit 97
STUB
...
STUB
  ...
  chmod +x "$case_dir/bin/bb" "$case_dir/bin/beagle-store-server" \
    "$case_dir/bin/beagle" "$case_dir/bin/ss" "$case_dir/bin/sleep"
...
run_up() {
  ...
  (
    ...
    PATH="$case_dir/bin:$PATH" \
      ...
      BEAGLE_STORE_SERVER_PORT=43129 \
      BEAGLE_STORE_SPACE_ID=test-space \
      BEAGLE_STORE_LOG="$log" \
      BEAGLE_STORE_STARTUP_TIMEOUT_SECONDS="$timeout_seconds" \
      "$case_dir/bin/beagle-store-up"
  )
}
...
grep -q "different BEAGLE_STORE_SPACE_ID" "$wrong_dir/state/output" ||
...
grep -q "BEAGLE_STORE_STARTUP_TIMEOUT_SECONDS must be an integer from 1 to 3600" \
...
echo "beagle-store-up readiness: PASS"
