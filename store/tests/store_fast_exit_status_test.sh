#!/usr/bin/env bash
# bin/north:684-706 dispatches on store-fast's exit status: 3 = unresolved
...
"$ROOT/../bin/beagle" store --help | grep -Fq 'Usage: beagle store' || {
  echo "store_fast_exit_status: beagle store help is unavailable" >&2
  exit 1
}
...
if [[ "$ready" -ne 1 ]]; then
  printf 'store_fast_exit_status: server failed to start\n' >&2
  sed -n '1,80p' "$scratch/server.log" >&2
  exit 1
fi

run_fast() {
  BEAGLE_STORE_SERVER_PORT="$1" BEAGLE_STORE_SPACE_ID=test-space \
    "$ROOT/../bin/beagle" store "${@:2}"
}
...
check_exit() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    printf 'store_fast_exit_status: %s expected exit %s, got %s\n' \
...
printf 'store_fast_exit_status: PASS\n'
