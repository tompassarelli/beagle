...
;; ============================================================================
;; Drives the graph-authored defcheck gate + bin/beagle-store-defcheck-server
...
(def arena "/tmp/store-a2-selftest")
(def code-log (str arena "/.store/code.log"))
...
;; bin/beagle-store-ingest-code resolves the Beagle-pinned racket itself and fails
;; loud without it; predict that here and SKIP (CI runners carry no toolchain).
(when-not (or (System/getenv "BEAGLE_STORE_RACKET")
              (let [bh (or (System/getenv "BEAGLE_HOME")
                    ...
                    p (try (str/trim (:out (proc/sh {:out :string :err :string}
                           ...
                           (catch Throwable _ ""))]
                (not (str/blank? p))))
  (println "SKIP — missing prerequisite: Beagle-pinned racket (set BEAGLE_STORE_RACKET, or direnv+beagle)")
  (System/exit 0))
(log! "ingesting arena modules ->" code-log)
(let [r (proc/sh {:dir repo} "bin/beagle-store-ingest-code" (str arena "/gw") "--out" code-log)]
...
(proc/sh {:dir repo :extra-env {"BEAGLE_STORE_SERVER_PORT" (str database-port) "BEAGLE_STORE_LOG" code-log}} "bin/beagle-store-up")
(Thread/sleep 3000)
...
(defn run []
  ...
  (let [st (database {:op :status})]
    (when-not (str/includes? (str (:log st)) "store-a2-selftest")
