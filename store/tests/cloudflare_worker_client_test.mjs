...
check('client requires an explicit SpaceId and closed input keys', () => {
  assert.throws(() => client.storeClient({ token: 'secret' }), /space required/);
...
check('version request is the exact JSON STORERPC envelope', async () => {
  ...
  const response = await store.version({ expectedVersion: 11n });
...
check('assert lowers directly to a typed recursive write record', async () => {
  ...
  await store.assert(client.tripleTerm('file', client.keywordTerm('page'), 1),
...
check('the closed client exposes every public operation', async () => {
  ...
  await store.version(); await store.status(); await store.validate(); await store.occurrences();
  await store.scan({ t1: 'a' }); await store.query(client.tripleQuery());
  await store.assert('a', 'p', 'r'); await store.retract('a', 'p', 'r');
  await store.batch([{ op: 'assert', t1: 'a', t2: 'p', t3: 'r' }]);
  await store.leaseAcquire('resource', 'holder', 1000);
  await store.leaseRenew(fence, 1000); await store.leaseRelease(fence); await store.leaseCheck(fence);
...
check('shim HTTP errors stay structured JSON', async () => {
  ...
  await assert.rejects(store.version(), error => error.status === 401
