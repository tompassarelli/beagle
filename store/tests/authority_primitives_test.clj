...
(require '[store.authority :as a]
         '[store.tools :as tools]
...
(def catalog {"catalogVersion" "store.graph-edit-tools/v1"
              "tools" served-tools})
...
(def entry-a {"moduleId" "src.store.authority" "sourcePath" "src/store/authority.bclj"})
...
(chk-reject "orphan AST rejected"
            #(a/normalize-module-manifest! "" "1" [] ["src.store.orphan"]))
...
(chk-reject "duplicate AST module ids rejected"
            #(a/normalize-module-manifest! "" "1" [entry-a]
                                           ["src.store.authority" "src.store.authority"]))
...
(defn unsigned-descriptor [manifest]
  {"descriptorVersion" "store.graph-edit-authority/v1"
   ...
   "runtime" {"sealVersion" "store.runtime-seal/v1" "system" "x86_64-linux"
   ...
   {"checkoutRoot" "/checkout" "sourceRoot" "/checkout/src"
    ...
    "codeLog" "/checkout/.store/code.log"
...
(def manifest-more
  (a/normalize-module-manifest! "src" "10"
                                [entry-a {"moduleId" "src.store.extra" "sourcePath" "src/store/extra.bclj"}] []))
...
(defn unsigned-receipt [row]
  (cond->
   {"receiptVersion" "store.graph-edit-phase-receipt/v1"
    ...
    "request" {"tool" "add-def"
               "argumentsDigest" (a/digest-json-no-number-no-null!
                                  {"module" "src.store.authority" "form" "(def x 1)"})}
    "module" entry-a
...
(let [results @checks
  ...
  (if (empty? failures)
    (println "\nstore.authority primitives:" (count results) "/" (count results) "PASS")
    (do
      (println "\nstore.authority primitives:" (count failures) "FAILED of" (count results))
      (System/exit 1))))
