...
backup_command="$repo/bin/beagle-store-backup"
driver="$repo/tests/store_backup_restore_driver.mjs"
space="beagle-store-backup-restore"

skip() {
  echo "store backup restore: SKIP ($*)"
  exit 0
}

fail() {
  echo "store backup restore: FAIL: $*" >&2
  exit 1
}

beagle="${BEAGLE_STORE_BEAGLE:-${BEAGLE_HOME:+$BEAGLE_HOME/bin/beagle}}"
[[ -n "$beagle" && -x "$beagle" ]] || skip "set BEAGLE_STORE_BEAGLE to a beagle CLI"
...
artifact="${BEAGLE_STORE_BACKUP_ARTIFACT:-}"
if [[ -z "$artifact" ]]; then
  artifact="$(BEAGLE_STORE_BEAGLE="$beagle" "$builder" --host server "${sources[@]}")" ||
...
grep -Fq 'beagle-store-backup/verify-receipt/v1' <<<"$verify_receipt" ||
...
echo "store backup restore: PASS manifest=$manifest_hash history=$history_hash version=$post_version"
