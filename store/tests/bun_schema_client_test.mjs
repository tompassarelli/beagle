...
check('batch preflight reports exact deterministic request metrics and is rechecked before I/O', async () => {
  const disconnected = storeClient({ space: 'preflight-test', port: 1 });
  const actions = [
...
check('createUnique pins its identity read and guards creation with the same expected version', async () => {
  ...
  assertPinnedReads(store.calls, [7n]);
  assert.equal(store.calls.preflightBatch.length, 1);
  assert.equal(store.calls.preflightBatch[0].options.expectedVersion, 7n);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 7n);
  assert.equal(
    store.calls.batch[0].options.preflight,
    store.calls.preflightBatch[0].preflight,
  );
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('replaceSingle retracts every value from one pinned snapshot and asserts exactly once', async () => {
  ...
  assertPinnedReads(store.calls, [11n]);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 11n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('replaceSingle preserves duplicate live occurrences across pinned pages', async () => {
  ...
  assertPinnedReads(store.calls, [12n]);
  assert.equal(store.calls.scan.length, 2);
  assert.deepEqual(store.calls.scan[0].pattern, { t1: PAGE_A, t2: TITLE });
  assert.deepEqual(store.calls.scan[0].options, {
    page: { limit: 128 },
  });
  assert.deepEqual(store.calls.scan[1].options, {
    page: { limit: 128, cursor },
  });
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 12n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_A] },
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_A] },
    { op: 'assert', terms: [PAGE_A, TITLE, NEW_TITLE] },
...
check('an initial scan/version skew retries without sending a stale batch', async () => {
  ...
  const schema = schemaClient(store.client, { maxConflictRetries: 1 });

  await schema.replaceSingle(PAGE_A, TITLE, NEW_TITLE);

  assertPinnedReads(store.calls, [70n, 71n]);
  assert.equal(store.calls.version.length, 2);
  assert.equal(store.calls.scan.length, 2);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 71n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_B] },
    { op: 'assert', terms: [PAGE_A, TITLE, NEW_TITLE] },
  ]);
...
check('an OCC race re-reads a fresh pinned snapshot before retrying', async () => {
  ...
  assertPinnedReads(store.calls, [20n, 21n]);
  assert.deepEqual(
    store.calls.batch.map(call => call.options.expectedVersion),
    [20n, 21n],
  );
});
...
check('createUnique rejects an identity already owned by another subject without mutation', async () => {
  ...
  assertPinnedReads(store.calls, [30n]);
  assert.equal(store.calls.batch.length, 0);
});
...
check('upsertUnique reuses the sole identity match as the write subject', async () => {
  ...
  assertPinnedReads(store.calls, [40n]);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 40n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('updateUnique rejects a missing or duplicate source before scanning or writing', async () => {
  ...
  for (const [index, testCase] of cases.entries()) {
    ...
    assertPinnedReads(store.calls, [version]);
    assert.equal(store.calls.scan.length, 0);
    assert.equal(store.calls.batch.length, 0);
  }
});
...
check('multi allowedCurrent accepts reordered sets and duplicate occurrences', async () => {
  ...
  await schemaClient(store.client).updateUnique(updateInput({
    predicate: TAG,
    cardinality: 'multi',
    values: [NEW_TITLE],
  ...
  assertPinnedReads(store.calls, [81n]);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('single update accepts duplicate allowed current occurrences and replaces every occurrence at one version', async () => {
  ...
  assertPinnedReads(store.calls, [85n]);
  assert.equal(store.calls.query.length, 2);
  assert.equal(store.calls.scan.length, 1);
  assert.equal(store.calls.scan[0].versionAtCall, 85n);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 85n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_A] },
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_A] },
    { op: 'assert', terms: [PAGE_A, TITLE, NEW_TITLE] },
...
check('an update OCC conflict reruns source, required identities, and current guards', async () => {
  ...
  assertPinnedReads(store.calls, [100n, 101n]);
  assert.equal(store.calls.query.length, 4);
  assert.equal(store.calls.scan.length, 2);
  assert.deepEqual(store.calls.batch.map(call => call.options.expectedVersion), [100n, 101n]);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_A] },
    { op: 'assert', terms: [PAGE_A, TITLE, NEW_TITLE] },
  ]);
  assert.deepEqual(semanticActions(store.calls.batch[1].actions), [
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_B] },
    { op: 'assert', terms: [PAGE_A, TITLE, NEW_TITLE] },
  ]);
});
...
check('an update scan skew reruns every guard and never submits the stale attempt', async () => {
  ...
  assertPinnedReads(store.calls, [110n, 111n]);
  assert.equal(store.calls.query.length, 4);
  assert.equal(store.calls.scan.length, 2);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 111n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
    { op: 'retract', terms: [PAGE_A, TITLE, OLD_TITLE_B] },
    { op: 'assert', terms: [PAGE_A, TITLE, NEW_TITLE] },
  ]);
...
check('updateUniqueMany replaces multiple fields and subjects in one pinned batch', async () => {
  ...
  const result = await schemaClient(store.client).updateUniqueMany(publicationInput({
    requireUnique: [requiredAuthor()],
  }));

  assertPinnedReads(store.calls, [140n]);
  assert.deepEqual(result.subjects, [REVISION_A, PAGE_A]);
  assert.equal(result.changed, true);
  assert.equal(result.servedVersion, 141n);
  assert.equal(store.calls.query.length, 3);
  assert.equal(store.calls.scan.length, 3);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 140n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('empty allowedCurrent is an absence CAS and one failed cell prevents the whole command', async () => {
  ...
  await assert.rejects(
    schemaClient(store.client).updateUniqueMany(publicationInput()),
    error => error instanceof SchemaConstraintError
      && error.code === 'schema/current-value-rejected'
...
check('a later multi-update scan skew replans every source and field', async () => {
  ...
  await schemaClient(store.client, { maxConflictRetries: 1 })
    .updateUniqueMany(publicationInput());

  assert.deepEqual(store.calls.query.map(call => call.options.asOf), [150n, 150n, 151n, 151n]);
  assert.equal(store.calls.scan.length, 6);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 151n);
});
...
check('a multi-update conflict rereads and rebuilds the whole command', async () => {
  ...
  assert.deepEqual(store.calls.batch.map(call => call.options.expectedVersion), [152n, 153n]);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions).slice(0, 2), [
    { op: 'retract', terms: [REVISION_A, REVISION_STATUS, DRAFT] },
    { op: 'assert', terms: [REVISION_A, REVISION_STATUS, CANONICAL] },
  ]);
  assert.deepEqual(semanticActions(store.calls.batch[1].actions), [
...
check('createUnique checks required identities at the same version as creation', async () => {
  ...
  assertPinnedReads(store.calls, [120n]);
  assert.equal(store.calls.query.length, 2);
  assert.equal(store.calls.scan.length, 0);
  assert.equal(store.calls.batch.length, 1);
  assert.equal(store.calls.batch[0].options.expectedVersion, 120n);
});
...
check('requireUnique resolution keeps query concurrency bounded', async () => {
  ...
  const mutation = schemaClient(store).createUnique({
    subject: PAGE_A,
    identity: { predicate: SLUG, value: HOME },
    fields: [],
    requireUnique,
...
check('upsertUnique keeps single-field scan concurrency bounded', async () => {
  ...
  const mutation = schemaClient(store).upsertUnique({
    subject: PAGE_B,
    identity: { predicate: SLUG, value: HOME },
    fields,
  });
...
check('two scan pages can expose the 248th occurrence action-limit sentinel', async () => {
  ...
  assert.equal(store.calls.scan.length, SCHEMA_MAX_READ_PAGES);
  assert.equal(store.calls.batch.length, 0);
});
...
check('transactUnique creates a mutually-referencing set with planned uniqueness guards', async () => {
  ...
  const result = await schemaClient(store.client).transactUnique({
    creates: [
      {
        subject: PAGE_A,
  ...
  assert.equal(store.calls.scan.length, 0);
  assert.equal(store.calls.preflightBatch.length, 1);
  assert.equal(result.preflight, store.calls.preflightBatch[0].preflight);
  assert.equal(store.calls.batch[0].options.preflight, result.preflight);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('transactUnique composes an exact multi-set guard with required Ref identities', async () => {
  ...
  assert.deepEqual(result.updatedSubjects, [PAGE_A]);
  assertPinnedReads(store.calls, [172n]);
  assert.equal(store.calls.batch[0].options.expectedVersion, 172n);
  assert.deepEqual(semanticActions(store.calls.batch[0].actions), [
...
check('an already-empty single clear is an update-only transaction no-op', async () => {
  ...
  assert.equal(store.calls.preflightBatch.length, 0);
  assert.equal(store.calls.batch.length, 0);
});
...
check('a planned idempotency reservation is re-resolved after conflict before any second write', async () => {
  ...
  await assert.rejects(
    schemaClient(store.client).transactUnique({
      creates: [
        {
          subject: PAGE_C,
...
check('non-conflict write errors propagate unchanged and are not retried', async () => {
  ...
  const schema = schemaClient(store.client, { maxConflictRetries: 4 });
  ...
  assert.equal(store.calls.version.length, 1);
  assert.equal(store.calls.batch.length, 1);
});
