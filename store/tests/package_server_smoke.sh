...
if "$env_bin" -i BEAGLE_STORE_SPACE_ID="$space" \
  ...
  echo "store package smoke: default launch silently fell back from native" >&2
  exit 1
fi
"$grep_bin" -Fxq \
  "beagle-store-server: BEAGLE_STORE_NATIVE_ARTIFACT_DIR is required for BEAGLE_STORE_SERVER_RUNTIME=native" \
  "$native_error" || {
...
if "$env_bin" -i BEAGLE_STORE_SPACE_ID="$space" BEAGLE_STORE_SERVER_RUNTIME=graal \
    "$package_root/bin/beagle-store-server" serve "$port" "$log" \
    >"$graal_error" 2>&1; then
  echo "store package smoke: Graal launch silently ran without an artifact" >&2
  exit 1
fi
"$grep_bin" -Fxq \
  "beagle-store-server: BEAGLE_STORE_GRAAL_ARTIFACT is required for BEAGLE_STORE_SERVER_RUNTIME=graal" \
  "$graal_error" || {
    echo "store package smoke: missing Graal artifact did not fail exactly" >&2
...
start_server() {
  (
    ...
    exec "$env_bin" -i HOME="$home" XDG_CACHE_HOME="$home/.cache" \
      ...
      BEAGLE_STORE_SERVER_RUNTIME=jvm-oracle \
      "$package_root/bin/beagle-store-server" serve "$port" "$log"
  ) >"$server_output" 2>&1 &
  pid=$!
}
...
wait_ready() {
  ...
  echo "store package smoke: no native version response" >&2
  sed -n '1,160p' "$server_output" >&2
  return 1
}
...
if ! "$grep_bin" -Fq '"isError":false' <<<"$mcp_output"; then
  echo "store package smoke: MCP native tell failed" >&2
