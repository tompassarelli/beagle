...
;; ============================================================================
;; Boots a code daemon with BEAGLE_STORE_DEFCHECK=1 so serve wires the stateful gate API
...
;;
;;   BEAGLE_STORE_DEFCHECK=1 clojure -M tests/code_write_def_realcheck_test.clj > out 2>&1; echo EXIT=$?
;; ============================================================================
(require '[clojure.string :as str] '[clojure.java.io :as io])

(def root (System/getProperty "user.dir"))
(def code-log (str root "/.store/code.log"))
(when-not (.exists (io/file code-log))
  (println "SKIP — no .store/code.log") (System/exit 0))
(when-not (= "1" (System/getenv "BEAGLE_STORE_DEFCHECK"))
  (println "SKIP — set BEAGLE_STORE_DEFCHECK=1 to run the warm def-check integration") (System/exit 0))
...
(boot-flat! flat)
(def server (future (serve port)))         ; serve calls maybe-wire-def-check! (BEAGLE_STORE_DEFCHECK=1)
(Thread/sleep 900)
...
(cond
  ...
  (do (println "SKIP — BEAGLE_STORE_DEFCHECK wiring did not activate (deep-check" (:deep-check probe) ")") (shutdown!) (System/exit 0))
  :else
  (do
