...
#
# Kill-switch: env AGENT_NO_AUTHORING_HOOKS (or CLAUDE_NO_AUTHORING_HOOKS
# compatibility alias); any value but 0/false = OFF, 0/false forces guards
# live. Owner-local, env-only implementation — no shared state file.
set -uo pipefail
umask 077
...
# Kill-switch, owner-local (env-only; any value but 0/false kills this
# session; 0/false forces guards live). Self-contained — no shared state file,
# no north-config dependency. Smallest injection seam: set
# BEAGLE_AUTHORING_KILLSWITCH_LIB to a script defining `authoring_guards_off`
# before this line to override with a caller-supplied policy.
