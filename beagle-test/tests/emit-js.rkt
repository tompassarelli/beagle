...
(run-tests
 (test-suite "JS emitter"
   ...
   (check-js-contains "defscalar equality predicate emits comparison, not assignment"
     "v === 0"
     '(defscalar Zero Int :where (= 0)))

   (check-js-contains "defscalar inequality predicate emits valid JS"
     "v !== 0"
   ...
   (test-case "imported record updates call the provider-owned validator"
     ...
     (define provider-stxs
       (map (lambda (form) (datum->syntax #f form)) provider-datums))
     (define provider
       (parse-program provider-stxs #:source-path "interface/provider.bjs"))
     (type-check! provider)
     ...
     (define consumer
       (parse-program
        (map (lambda (form) (datum->syntax #f form)) consumer-datums)
        #:source-path "interface/consumer.bjs"
        #:module-resolver
        (lambda (namespace _importer-source)
          (and (eq? namespace 'interface.provider) provider-source))))
     (type-check! consumer)
     ...
     (check-true
      (string-contains?
       result
       "Object.freeze(p.$beagle$record$Character$validate({")
      (format "provider-owned validator did not use the import alias:\n~a" result))
     (check-equal?
      (length
       (regexp-match*
        (regexp
         (regexp-quote "p.$beagle$record$Character$validate("))
        result))
      1
      (format "provider validator must be called exactly once:\n~a" result))

     (define referred-consumer-datums
       (list
     ...
     (define referred-consumer
       (parse-program
        (map (lambda (form) (datum->syntax #f form))
             referred-consumer-datums)
        #:source-path "interface/referred_consumer.bjs"
        #:module-resolver
        (lambda (namespace _importer-source)
          (and (eq? namespace 'interface.provider) provider-source))))
     (type-check! referred-consumer)
     (define referred-result (emit-program referred-consumer))
     (check-true
      (string-contains?
       referred-result
     ...
     (check-true
      (string-contains?
       referred-result
       "Object.freeze($beagle$record$Character$validate({")
      (format "referred provider validator was not called:\n~a"
              referred-result))
     (check-equal?
      (length
       (regexp-match*
        (regexp
         (regexp-quote "$beagle$record$Character$validate("))
        referred-result))
      1
...
(test-case "JS-translated function emits no warning"
   ...
   (check-js-contains "let :as in return position binds whole map from the value"
     "let whole = $beagle$binding$0;"
     `(defn f ,(br '(m Any)) Any
        (let (,(mt ':keys (br 'a) ':as 'whole) m) whole)))

   (check-js-contains "let :as destructures leaves from the same single source"
     "let a = $beagle$binding$0[\"a\"];"
     `(defn f ,(br '(m Any)) Any
        (let (,(mt ':keys (br 'a) ':as 'whole) m) whole)))
   ...
   (check-js-contains "assoc with compound key -> hamtMapAssoc"
     "hamtMapAssoc("
     `(defn f ,(br) Any (assoc ,(mt) ,(mt ':k 1) "a")))
   ...
   (check-js-contains "(set [compound]) -> hamtSet"
     "hamtSet("
     `(defn f ,(br) Any (set ,(br (br 1 2) (br 1 2)))))

   (check-js-contains "count of value-set -> hamtSetCount"
     "hamtSetCount("
     `(defn f ,(br) Int (count (set ,(br (br 1 2))))))
