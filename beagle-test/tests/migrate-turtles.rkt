...
;; Use a readtable that treats `|` like the ordinary char `a` so turtles
;; output containing `|>` and `|>>` reads back cleanly.
...
(defn add [(x : Int) (y : Int)] : Int (+ x y))"
  '((claim add ∈ (→ (Int Int) Int))
    (defn add (params x y) (body (+ x y))))
  "defn with typed params and return type")

(check-migrate
  "#lang beagle
(defn id [x] x)"
  '((defn id (params x) (body x)))
  "defn with no types")
...
(defn add [(x : Int) (y : Int)] : Int (+ x y) (println \"side\"))"
  '((claim add ∈ (→ (Int Int) Int))
    (defn add (params x y) (body (+ x y) (println "side"))))
...
(defn f [] : Int (let [x 1 y 2] (+ x y)))"
  '((claim f ∈ (→ () Int))
    (defn f (params) (body (let ((x 1) (y 2)) (body (+ x y))))))
...
(defn classify [(n : Int)] : String
  ...
  '((claim classify ∈ (→ (Int) String))
    (defn classify
      (params n)
      (body (cond ((< n 0) "neg") ((= n 0) "zero") (:else "pos")))))
  "cond — flat pairs become parenthesized pairs")
