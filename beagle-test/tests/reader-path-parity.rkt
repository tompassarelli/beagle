...
(test-case "structural containers retain exact delimiter and child source spans"
  (define source "[\n  alpha\n  {:k beta}\n  #{gamma}\n]")
  (define in (open-input-string source))
  (port-count-lines! in)
  (define outer (beagle-read-syntax "container-spans.bgl" in))
  (define elems (cdr (syntax->list outer)))
  (define map-stx (cadr elems))
  (define set-stx (caddr elems))
  (define map-elems (cdr (syntax->list map-stx)))
  (define set-elems (cdr (syntax->list set-stx)))

  ;; Spans include both delimiters; nested forms keep their own coordinates.
  (check-equal? (list (syntax-line outer)
                      (syntax-column outer)
                      (syntax-position outer)
                      (syntax-span outer))
                (list 1 0 1 (string-length source)))
  (check-equal? (list (syntax-line (car elems))
                      (syntax-column (car elems))
                      (syntax-position (car elems))
                      (syntax-span (car elems)))
                '(2 2 5 5))
  (check-equal? (list (syntax-line map-stx)
                      (syntax-column map-stx)
                      (syntax-position map-stx)
                      (syntax-span map-stx))
                '(3 2 13 9))
  (check-equal? (list (syntax-line (cadr map-elems))
                      (syntax-column (cadr map-elems))
                      (syntax-position (cadr map-elems))
                      (syntax-span (cadr map-elems)))
                '(3 6 17 4))
  (check-equal? (list (syntax-line set-stx)
                      (syntax-column set-stx)
                      (syntax-position set-stx)
                      (syntax-span set-stx))
                '(4 2 25 8))
  (check-equal? (list (syntax-line (car set-elems))
                      (syntax-column (car set-elems))
                      (syntax-position (car set-elems))
                      (syntax-span (car set-elems)))
                '(4 4 27 5)))
