...
(test-case "macro produces wrong-typed literal → rebucketed as macro-expansion-type-error"
  ;; (defmacro bad [] `"hello")
  ;; (def y Int (bad))
  ;; bad expands to "hello" — parses fine but type-checks fail (the positional
  ;; type says Int, while the value is String). Inside the macro-derived
...
(test-case "macro expansion inherits the call-site source position"
  ...
  (define def-stx
    (datum->syntax #f (list 'def 'y 'Int mk-call)
