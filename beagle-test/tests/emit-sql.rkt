...
(test-case "deftable emits CREATE TABLE"
  (define out (sql-emit "(define-target sql) (sql/deftable products [(id : Int :primary-key) (name : String :not-null)])"))
...
(test-case "deftable nullable column skips NOT NULL"
  (define out (sql-emit "(define-target sql) (sql/deftable items [(id : Int) (note : (U String Nil))])"))
...
(test-case "deftable with unique constraint"
  (define out (sql-emit "(define-target sql) (sql/deftable users [(id : Int :primary-key) (email : String :unique)])"))
  (check-true (string-contains? out "\"email\" TEXT NOT NULL UNIQUE")))

(test-case "deftable Float maps to REAL"
  (define out (sql-emit "(define-target sql) (sql/deftable prices [(amount : Float)])"))
  (check-true (string-contains? out "\"amount\" REAL NOT NULL")))

(test-case "deftable Bool maps to BOOLEAN"
  (define out (sql-emit "(define-target sql) (sql/deftable flags [(active : Bool)])"))
...
(test-case "simple select"
  (define out (sql-emit "(define-target sql) (sql/select [name price] (from products) (where (= category \"electronics\")))"))
...
(test-case "select with column refs"
  (define out (sql-emit "(define-target sql) (sql/select [p.id p.name] (from products :as p))"))
...
(test-case "select with numeric where"
  (define out (sql-emit "(define-target sql) (sql/select [name] (from products) (where (> price 10)))"))
  (check-true (string-contains? out "SELECT \"name\""))
  (check-true (string-contains? out "WHERE \"price\" > 10")))

(test-case "select star"
  (define out (sql-emit "(define-target sql) (sql/select [*] (from products))"))
...
(test-case "select distinct"
  (define out (sql-emit "(define-target sql) (sql/select-distinct [category] (from products))"))
...
(test-case "select with left join"
  (define out (se "(define-target sql) " "(sql/select [p.id p.name (count o.id :as order_count)] " "(from products :as p) " "(left-join orders :as o (= o.product_id p.id)) " "(where (> p.stock 0)) " "(group-by p.id p.name))"))
...
(test-case "select with full join"
  (define out (se "(define-target sql) " "(sql/select [a.id b.id] (from tableA :as a) (full-join tableB :as b (= a.key b.key)))"))
  (check-true (string-contains? out "FULL JOIN \"tableB\" AS \"b\" ON \"a\".\"key\" = \"b\".\"key\"")))

(test-case "select with cross join"
  (define out (se "(define-target sql) " "(sql/select [a.x b.y] (from colors :as a) (cross-join sizes :as b))"))
...
(test-case "select with order-by and limit"
  (define out (se "(define-target sql) " "(sql/select [name price] (from products) (order-by price :desc) (limit 10))"))
  (check-true (string-contains? out "SELECT \"name\", \"price\""))
  (check-true (string-contains? out "ORDER BY \"price\" DESC"))
  (check-true (string-contains? out "LIMIT 10")))

(test-case "select with offset"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (limit 10) (offset 20))"))
  (check-true (string-contains? out "LIMIT 10"))
...
(test-case "insert with values"
  (define out (se "(define-target sql) " "(sql/deftable products [(name : String) (price : Float) (stock : Int)]) " "(sql/insert products [name price stock] (values [\"Widget\" 9.99 100] [\"Gadget\" 19.99 50]))"))
...
(test-case "update with set and where"
  (define out (se "(define-target sql) " "(sql/deftable products [(price : Float) (stock : Int) (category : String)]) " "(sql/update products (set [price (* price 0.9)] [stock (+ stock 10)]) (where (= category \"clearance\")))"))
...
(test-case "delete with where"
  (define out (se "(define-target sql) " "(sql/deftable products [(total : Float)]) " "(sql/delete products (where (< total 0)))"))
...
(test-case "is-null and is-not-null"
  (define out (se "(define-target sql) " "(sql/select [name] (from users) (where (is-null email)))"))
  (check-true (string-contains? out "WHERE \"email\" IS NULL")))

(test-case "CASE expression"
  (define out (se "(define-target sql) " "(sql/select [(case (when (> price 100) \"expensive\") (when (> price 50) \"medium\") :else \"cheap\") :as tier] (from products))"))
...
(test-case "CAST expression"
  (define out (se "(define-target sql) " "(sql/select [(cast price integer)] (from products))"))
  (check-true (string-contains? out "CAST(\"price\" AS INTEGER)")))

(test-case "where with and/or"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (and (> price 5) (< stock 100))))"))
  (check-true (string-contains? out "WHERE (\"price\" > 5 AND \"stock\" < 100)")))

(test-case "string concatenation with ||"
  (define out (se "(define-target sql) " "(sql/select [(|| first_name \" \" last_name) :as full_name] (from users))"))
  (check-true (string-contains? out "\"first_name\" || ' ' || \"last_name\"")))

(test-case "BETWEEN expression"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (between price 10 50)))"))
  (check-true (string-contains? out "WHERE \"price\" BETWEEN 10 AND 50")))

(test-case "IN expression with values"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (in category \"electronics\" \"books\" \"toys\")))"))
  (check-true (string-contains? out "WHERE \"category\" IN ('electronics', 'books', 'toys')")))

(test-case "LIKE expression"
  (define out (se "(define-target sql) " "(sql/select [name] (from users) (where (like name \"%smith%\")))"))
  (check-true (string-contains? out "WHERE \"name\" LIKE '%smith%'")))

(test-case "NOT expression"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (not (= active #f))))"))
...
(test-case "select with multiple aggregates"
  (define out (se "(define-target sql) " "(sql/select [(count id) (avg price :as avg_price)] (from products))"))
  (check-true (string-contains? out "COUNT(\"id\")"))
  (check-true (string-contains? out "AVG(\"price\") AS \"avg_price\"")))

(test-case "count distinct"
  (define out (se "(define-target sql) " "(sql/select [(count-distinct category)] (from products))"))
  (check-true (string-contains? out "COUNT(DISTINCT \"category\")")))

(test-case "count star"
  (define out (se "(define-target sql) " "(sql/select [(count *)] (from products))"))
  (check-true (string-contains? out "COUNT(*)")))

(test-case "sum and min/max"
  (define out (se "(define-target sql) " "(sql/select [(sum price :as total) (min price :as cheapest) (max price :as priciest)] (from products))"))
...
(test-case "row_number window function"
  (define out (se "(define-target sql) " "(sql/select [name (row_number :as rn :over (order-by price :asc))] (from products))"))
  (check-true (string-contains? out "ROW_NUMBER() OVER (ORDER BY \"price\" ASC) AS \"rn\"")))

(test-case "sum window function with partition"
  (define out (se "(define-target sql) " "(sql/select [category (sum price :as running :over (partition-by category) (order-by name :asc))] (from products))"))
...
(test-case "rank window function"
  (define out (se "(define-target sql) " "(sql/select [name (rank :as rnk :over (order-by score :desc))] (from players))"))
...
(test-case "EXISTS subquery"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (exists (sql/select [1] (from orders) (where (= orders.product_id products.id))))))"))
...
(test-case "IN subquery"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (in id (sql/select [product_id] (from orders)))))"))
...
(test-case "single CTE"
  (define out (se "(define-target sql) " "(sql/with (recent_orders (sql/select [*] (from orders) (where (> created_at \"2024-01-01\")))) (sql/select [*] (from recent_orders)))"))
...
(test-case "multiple CTEs"
  (define out (se "(define-target sql) " "(sql/with (big (sql/select [id] (from products) (where (> price 100)))) (cheap (sql/select [id] (from products) (where (< price 10)))) (sql/select [*] (from big)))"))
...
(test-case "UNION"
  (define out (se "(define-target sql) " "(sql/union (sql/select [name] (from products)) (sql/select [name] (from archived_products)))"))
  (check-true (string-contains? out "UNION"))
  (check-true (string-contains? out "FROM \"products\""))
  (check-true (string-contains? out "FROM \"archived_products\"")))

(test-case "UNION ALL"
  (define out (se "(define-target sql) " "(sql/union-all (sql/select [name] (from products)) (sql/select [name] (from archived_products)))"))
  (check-true (string-contains? out "UNION ALL")))

(test-case "INTERSECT"
  (define out (se "(define-target sql) " "(sql/intersect (sql/select [name] (from products)) (sql/select [name] (from featured_products)))"))
  (check-true (string-contains? out "INTERSECT")))

(test-case "EXCEPT"
  (define out (se "(define-target sql) " "(sql/except (sql/select [id] (from all_users)) (sql/select [id] (from banned_users)))"))
...
(test-case "INSERT...SELECT"
  (define out (se "(define-target sql) " "(sql/insert-select archive [name price] (sql/select [name price] (from products) (where (= discontinued #t))))"))
  (check-true (string-contains? out "INSERT INTO \"archive\" (\"name\", \"price\")"))
  (check-true (string-contains? out "SELECT \"name\", \"price\""))
  (check-true (string-contains? out "FROM \"products\"")))

(test-case "RETURNING clause"
  (define out (se "(define-target sql) " "(sql/deftable users [(id : Int) (name : String)]) " "(sql/returning (sql/insert users [name] (values [\"Alice\"])) [id name])"))
...
(test-case "insert to unknown table raises error"
  (define err (sce "(define-target sql) " "(sql/deftable users [(id : Int) (name : String)]) " "(sql/insert products [name] (values [\"x\"]))"))
  (check-true (and (string? err) (string-contains? err "unknown table products"))))

(test-case "insert with unknown column raises error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int) (name : String)]) " "(sql/insert products [id name bogus] (values [1 \"x\" 42]))"))
  (check-true (and (string? err) (string-contains? err "unknown column bogus"))))

(test-case "update to unknown table raises error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int) (price : Float)]) " "(sql/update orders (set [price 10]) (where (= id 1)))"))
  (check-true (and (string? err) (string-contains? err "unknown table orders"))))

(test-case "update with unknown column raises error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int) (price : Float)]) " "(sql/update products (set [bogus 10]) (where (= id 1)))"))
  (check-true (and (string? err) (string-contains? err "unknown column bogus"))))

(test-case "delete from unknown table raises error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int)]) " "(sql/delete orders (where (= id 1)))"))
  (check-true (and (string? err) (string-contains? err "unknown table orders"))))

(test-case "select with validated column refs passes"
  (define out (se "(define-target sql) " "(sql/deftable products [(id : Int) (name : String) (price : Float)]) " "(sql/select [p.id p.name] (from products :as p))"))
  (check-true (string-contains? out "SELECT \"p\".\"id\", \"p\".\"name\"")))

(test-case "select with invalid column ref raises error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int) (name : String)]) " "(sql/select [p.id p.bogus] (from products :as p))"))
...
(test-case "insert string into Int column raises type error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int) (name : String)]) " "(sql/insert products [id name] (values [\"not-a-number\" \"Widget\"]))"))
  (check-true (and (string? err) (string-contains? err "expected Int"))))

(test-case "insert int into String column raises type error"
  (define err (sce "(define-target sql) " "(sql/deftable products [(id : Int) (name : String)]) " "(sql/insert products [id name] (values [1 42]))"))
  (check-true (and (string? err) (string-contains? err "expected String"))))

(test-case "insert correct types passes"
  (define out (se "(define-target sql) " "(sql/deftable products [(id : Int) (name : String) (price : Float)]) " "(sql/insert products [id name price] (values [1 \"Widget\" 9.99]))"))
...
(test-case "select in clj target raises error"
  (define err (sql-check-error "(define-target clj) (sql/select [name] (from products))"))
  (check-true (and (string? err) (string-contains? err "only supported in beagle/sql"))))

(test-case "deftable in js target raises error"
  (define err (sql-check-error "(define-target js) (sql/deftable users [(id : Int)])"))
  (check-true (and (string? err) (string-contains? err "only supported in beagle/sql"))))

(test-case "insert in nix target raises error"
  (define err (sql-check-error "(define-target nix) (sql/insert products [name] (values [\"x\"]))"))
  (check-true (and (string? err) (string-contains? err "only supported in beagle/sql"))))
...
(test-case "select with having"
  (define out (se "(define-target sql) " "(sql/select [category (count id :as cnt)] (from products) (group-by category) (having (> (count id) 5)))"))
...
(test-case "upper/lower string functions"
  (define out (se "(define-target sql) " "(sql/select [(upper name) (lower email)] (from users))"))
  (check-true (string-contains? out "UPPER(\"name\")"))
  (check-true (string-contains? out "LOWER(\"email\")")))

(test-case "coalesce function"
  (define out (se "(define-target sql) " "(sql/select [(coalesce nickname name)] (from users))"))
  (check-true (string-contains? out "COALESCE(\"nickname\", \"name\")")))

(test-case "substring function"
  (define out (se "(define-target sql) " "(sql/select [(substring name 1 3)] (from users))"))
...
(define ecommerce-schema
  (string-append
    "(define-target sql) "
    "(sql/deftable customers [(id : Int :primary-key) (name : String :not-null) (email : String :unique)]) "
    "(sql/deftable products [(id : Int :primary-key) (name : String :not-null) (price : Float :not-null) (category : String)]) "
    "(sql/deftable orders [(id : Int :primary-key) (customer_id : Int) (order_date : String) (status : String)]) "
    "(sql/deftable order_items [(id : Int :primary-key) (order_id : Int) (product_id : Int) (quantity : Int) (unit_price : Float)]) "
    "(sql/deftable reviews [(id : Int :primary-key) (product_id : Int) (customer_id : Int) (rating : Int) (body : String)]) "))
...
(test-case "e-commerce: revenue by category with window ranking"
  (define out (se ecommerce-schema "(sql/select [p.category (sum oi.unit_price :as revenue) " "(rank :as rev_rank :over (order-by (sum oi.unit_price) :desc))] " "(from order_items :as oi) " "(left-join products :as p (= oi.product_id p.id)) " "(group-by p.category))"))
...
(test-case "e-commerce: CTE with aggregation"
  (define out (se ecommerce-schema "(sql/with " "(top_products (sql/select [p.id p.name (avg r.rating :as avg_rating)] " "(from products :as p) " "(left-join reviews :as r (= r.product_id p.id)) " "(group-by p.id p.name) " "(having (> (avg r.rating) 4)))) " "(sql/select [*] (from top_products) (order-by avg_rating :desc)))"))
...
(test-case "e-commerce: CASE with status classification"
  (define out (se ecommerce-schema "(sql/select [o.id (case (when (= o.status \"shipped\") \"in transit\") " "(when (= o.status \"delivered\") \"complete\") " ":else \"pending\") :as display_status] " "(from orders :as o))"))
  (check-true (string-contains? out "CASE"))
...
(test-case "e-commerce: NOT EXISTS anti-join"
  (define out (se ecommerce-schema "(sql/select [c.name c.email] (from customers :as c) " "(where (not (exists (sql/select [1] (from orders :as o) (where (= o.customer_id c.id)))))))"))
...
(test-case "e-commerce: union queries"
  (define out (se ecommerce-schema "(sql/union " "(sql/select [name price] (from products) (where (= category \"electronics\"))) " "(sql/select [name price] (from products) (where (= category \"books\"))))"))
  (check-true (string-contains? out "UNION"))
  (check-true (string-contains? out "WHERE \"category\" = 'electronics'"))
...
(test-case "multiple statements separated by semicolons"
  (define out (se "(define-target sql) " "(sql/deftable users [(id : Int :primary-key) (name : String)]) " "(sql/insert users [name] (values [\"Alice\"])) " "(sql/select [*] (from users))"))
...
(test-case "delete without where"
  (define out (se "(define-target sql) " "(sql/deftable temp [(id : Int)]) " "(sql/delete temp)"))
...
(test-case "select with multiple order-by columns"
  (define out (se "(define-target sql) " "(sql/select [name price] (from products) (order-by category :asc price :desc))"))
  (check-true (string-contains? out "ORDER BY \"category\" ASC, \"price\" DESC")))

(test-case "nested arithmetic in where"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (> (* price quantity) 1000)))"))
  (check-true (string-contains? out "WHERE \"price\" * \"quantity\" > 1000")))

(test-case "boolean literal values"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (= active #t)))"))
  (check-true (string-contains? out "WHERE \"active\" = TRUE")))

(test-case "insert-select to unknown table is permissive"
  (define out (se "(define-target sql) " "(sql/insert-select archive [name price] (sql/select [name price] (from products)))"))
  (check-true (string-contains? out "INSERT INTO \"archive\" (\"name\", \"price\")")))

(test-case "multi-column create index"
  (define out (se "(define-target sql) " "(sql/create-index idx_orders_cust_date orders [customer_id order_date])"))
...
(test-case "group-by: bare column not in group-by raises error"
  (define err (sce "(define-target sql) " "(sql/select [name (count id)] (from products) (group-by category))"))
...
(test-case "group-by: qualified column not in group-by raises error"
  (define err (sce "(define-target sql) " "(sql/select [p.name (count p.id)] (from products :as p) (group-by p.category))"))
  (check-true (and (string? err) (string-contains? err "must appear in GROUP BY or be an aggregate"))))

(test-case "group-by: all columns in group-by passes"
  (define out (se "(define-target sql) " "(sql/select [category (count id)] (from products) (group-by category))"))
  (check-true (string-contains? out "GROUP BY \"category\"")))

(test-case "group-by: qualified columns in group-by passes"
  (define out (se "(define-target sql) " "(sql/select [p.category (count p.id)] (from products :as p) (group-by p.category))"))
  (check-true (string-contains? out "GROUP BY \"p\".\"category\"")))

(test-case "group-by: aggregate alias passes"
  (define out (se "(define-target sql) " "(sql/select [category (count id :as total)] (from products) (group-by category))"))
  (check-true (string-contains? out "COUNT(\"id\") AS \"total\"")))

(test-case "group-by: select without group-by still works"
  (define out (se "(define-target sql) " "(sql/select [name price] (from products))"))
  (check-true (string-contains? out "SELECT \"name\", \"price\"")))

(test-case "group-by: all aggregates with group-by passes"
  (define out (se "(define-target sql) " "(sql/select [(count id) (avg price)] (from products) (group-by category))"))
  (check-true (string-contains? out "COUNT(\"id\")"))
...
(test-case "string literal with single quote is properly escaped"
  (define out (se "(define-target sql) " "(sql/select [name] (from users) (where (= name \"O'Brien\")))"))
  (check-true (string-contains? out "'O''Brien'")))

(test-case "identifier quoting prevents injection in table names"
  (define out (sql-emit "(define-target sql) (sql/drop-table my_table)"))
  (check-true (string-contains? out "\"my_table\"")))

(test-case "+inf.0 rejected as numeric literal"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (> price +inf.0)))"))
...
(test-case "+nan.0 rejected as numeric literal"
  (define out (se "(define-target sql) " "(sql/select [name] (from products) (where (> price +nan.0)))"))
...
(test-case "now() emits empty parens, not (*)"
  (define out (se "(define-target sql) " "(sql/select [(now)] (from t))"))
...
(test-case "current_timestamp emits niladic keyword (no parens) with alias"
  (define out (se "(define-target sql) " "(sql/select [(current_timestamp :as ts)] (from t))"))
...
(test-case "current_date emits niladic keyword"
  (define out (se "(define-target sql) " "(sql/select [(current_date)] (from t))"))
...
(test-case "count() with no arg still emits COUNT(*)"
  (define out (se "(define-target sql) " "(sql/select [(count)] (from t))"))
  (check-true (string-contains? out "COUNT(*)")))
