...
(defn- ^String emit-for-target [^String target prog]
  ...
  (= target "zig") (ez/emit-program! prog)
...
(defn -main [& args]
  ...
  :else (do
  (selfhost.rt/eprint "usage: selfhost.main [--target clj|js|nix|zig] ast|check|emit FILE, emit-from-ast < ast.json, or facts-roundtrip MODE FILE\n")
  (selfhost.rt/exit 2)))
  (flush)))
