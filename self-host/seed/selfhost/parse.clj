...
(defn run-tests! []
  ...
  (expect! "let typed map destructuring accepts an Any initializer" (let [node (parse-expr* ["let" [BRACKET-TAG [MAP-TAG ":keys" [BRACKET-TAG "exit" "output" "report"]] "Any" "initializer"] "report"])
   binding (nth (get node "bindings") 0)
   target (get binding "name")]
  (and (= (get node "node") "let") (= (get target "type") "map-destructure") (= (get target "keys") ["exit" "output" "report"]) (= (get (get binding "ann") "name") "Any") (= (get (get binding "value") "name") "initializer") (= (get (nth (get node "body") 0) "name") "report"))))
  ...
  (expect! "parse-type! shaped Regex app" (= (parse-type! ["Regex" ["HVec" "String" "String"]]) {"kind" "app" "name" "Regex" "args" [{"kind" "app" "name" "HVec" "args" [{"kind" "prim" "name" "String"} {"kind" "prim" "name" "String"}]}]}))
