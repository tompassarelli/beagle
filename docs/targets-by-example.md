...
Bare `#lang beagle` on `.bgl` selects the experimental Native Core path; `.bgl`
is not a neutral container for a later target choice. `beagle build --materializer c17|qbe|wasm
--out DIR FILE.bgl` first freezes `module.native-program`, then writes only the
selected projection. Materializers are replaceable outputs; a digest identifies
the recorded artifact and inputs but does not grant execution authority.
`wasm` requires `--abi wasm32`; it is currently the named
