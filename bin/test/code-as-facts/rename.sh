...
# 6d. wrapped `(name: Type)` annotation cascades (legal field/param surface)
...
chk "wrapped param (x: Thing) -> (x: Item)" "grep -qF '(x: Item)' <<<\"\$(racket \"$RT\" --render $RESOLVE_OUT/resolved-tsc.bclj.edn 2>/dev/null)\""
