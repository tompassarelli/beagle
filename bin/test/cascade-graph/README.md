...
moved **off text and onto the code-as-facts graph of recursive triples**.
...
The new cascade derives its call graph from recursive triples in Fram's store
(`beagle-callgraph` → `beagle-facts` → Fram store → Datalog transitive closure
over the live view). Every call binds the
...
repair runs on Fram's Triple store. The scope-correctness assertions gate in CI; the
