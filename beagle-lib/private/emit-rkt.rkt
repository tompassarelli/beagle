...
(define (mangle-name sym)
  ...
  (define out
    (string-replace
     (string-replace s "?" "-p")
     "!" "-bang"))
  (if (member out rkt-reserved-words) (string-append out "_") out))
...
(define (emit-core-call sym args)
  ...
  (case sym
    ...
    [(str)
     (if (= (length a) 1)
         (format "(~a->string ~a)" "number" (car a))
         (format "(string-append ~a)" (string-join a " ")))]
      ...
      (cond
        ...
        [(pat-record? pat)
         ...
         (define bind-strs
           (for/list ([b (in-list bindings)]
                      [i (in-naturals)])
             (define accessor (format "~a-~a" type-name (mangle-name b)))
