...
(define (arr element)
  (type-app 'Arr (list (p element))))

(define (array-ctor element)
  ;; Clojure's primitive-array constructors accept either a size or a source
  ;; collection, plus the established optional initialization arguments.  The
  ;; container result is precise even though that dynamic input surface is not.
  (type-fn (list (p 'Any)) (p 'Any) (arr element)))
...
(define STDLIB-CLJ
  (hash
   ...
   ;; JVM arrays are mutable typed containers.  Preserve their element type
   ;; through construction, length, reads, writes, and clones rather than
   ;; collapsing every primitive array operation to Any.
   'alength
   (poly-fn '(A)
            (list (type-app 'Arr (list (tv 'A))))
            (p 'Int))
   'aget
   (poly-fn '(A)
            (list (type-app 'Arr (list (tv 'A))) (p 'Int))
            (tv 'A))
   'aset
   (poly-fn '(A)
            (list (type-app 'Arr (list (tv 'A))) (p 'Int) (tv 'A))
            (tv 'A))
   'aclone
   (poly-fn '(A)
            (list (type-app 'Arr (list (tv 'A))))
            (type-app 'Arr (list (tv 'A))))
   ...
   'aset-boolean   (type-fn (list (arr 'Bool) (p 'Int) (p 'Bool)) #f (p 'Bool))
   'aset-byte      (type-fn (list (arr 'I8) (p 'Int) (p 'I8)) #f (p 'I8))
   'aset-char      (type-fn (list (arr 'Char) (p 'Int) (p 'Char)) #f (p 'Char))
   'aset-double    (type-fn (list (arr 'Float) (p 'Int) (p 'Float)) #f (p 'Float))
   'aset-float     (type-fn (list (arr 'F32) (p 'Int) (p 'F32)) #f (p 'F32))
   'aset-int       (type-fn (list (arr 'I32) (p 'Int) (p 'I32)) #f (p 'I32))
   'aset-long      (type-fn (list (arr 'Int) (p 'Int) (p 'Int)) #f (p 'Int))
   'aset-short     (type-fn (list (arr 'I16) (p 'Int) (p 'I16)) #f (p 'I16))
   ...
   'boolean-array  (array-ctor 'Bool)
   'booleans       (type-fn (list (p 'Any)) #f (arr 'Bool))
   'byte-array     (array-ctor 'I8)
   'bytes          (type-fn (list (p 'Any)) #f (arr 'I8))
   'char-array     (array-ctor 'Char)
   'chars          (type-fn (list (p 'Any)) #f (arr 'Char))
   'double-array   (array-ctor 'Float)
   'doubles        (type-fn (list (p 'Any)) #f (arr 'Float))
   'float-array    (array-ctor 'F32)
   'floats         (type-fn (list (p 'Any)) #f (arr 'F32))
   'int-array      (array-ctor 'I32)
   'ints           (type-fn (list (p 'Any)) #f (arr 'I32))
   'long-array     (array-ctor 'Int)
   'longs          (type-fn (list (p 'Any)) #f (arr 'Int))
   'object-array   (fn-of '(Any) 'Any)
   'short-array    (array-ctor 'I16)
   'shorts         (type-fn (list (p 'Any)) #f (arr 'I16))
   'chunk          (fn-of '(Any) 'Any)
