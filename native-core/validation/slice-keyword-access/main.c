...
int main(void) {
  ...
  native_m0_type_23 version_text_values[] = {{
      .tag = INT64_C(0),
      .payload = {
  ...
  native_m0_type_14 rejection = native_vec_new(
  ...
  rejection = native_vec_push(
      &arena, rejection, &rejection_item, INT64_C(8), _Alignof(uint64_t));
  native_m0_type_25 reject_values[] = {{
      .tag = INT64_C(3),
  ...
  native_map *versioned = native_map_from_arrays(
      ...
      sizeof version_values[0], _Alignof(native_m0_type_23),
  ...
  native_map *version_text = native_map_from_arrays(
      ...
      sizeof version_keys[0], _Alignof(uint64_t),
      sizeof version_text_values[0], _Alignof(native_m0_type_23),
      NATIVE_COLLECTION_EQ_KEYWORD);
  native_map *version_missing = native_map_from_arrays(
      ...
      _Alignof(native_m0_type_23), NATIVE_COLLECTION_EQ_KEYWORD);
  native_map *optional_error = native_map_from_arrays(
      ...
      _Alignof(native_m0_type_21), NATIVE_COLLECTION_EQ_KEYWORD);
  ...
  native_m0_type_19 optional_nil = native_m0_fn_8(optional_error);
  native_m0_type_19 optional_map = native_m0_fn_8(optional_empty);
  ...
  native_m0_type_20 other_key = native_m0_fn_11(present);
  native_m0_type_20 other_source = native_m0_fn_12(present, missing);
  if (other_key.tag != INT64_C(1) || other_source.tag != INT64_C(1)) {
    return 9;
  }
  native_m0_type_20 false_missing = native_m0_fn_13(missing);
  native_m0_type_20 false_present = native_m0_fn_13(present);
  if (false_missing.tag != INT64_C(1) ||
      false_present.tag != INT64_C(0) ||
      false_present.payload.variant_0 != INT64_C(-1)) {
    return 10;
  }
  native_m0_type_3 provider_text = (uint64_t)(uintptr_t)&unknown_op;
  native_m0_type_5 provider_pair = {provider_text};
