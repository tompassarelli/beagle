...
artifact_names=(rt_core.facts inventory.txt managed.out report.txt frontier.txt source.sha256)
...
[[ -f "$here/main.c" ]] || die "complete C17 materialization has no native probe main.c"
build="$scratch/c"
mkdir -p "$build"
...
cmp -s "$scratch/gcc.out" "$generated/managed.out" \
  || die "GCC C17 probe differs from the managed oracle"
cmp -s "$scratch/clang.out" "$generated/managed.out" \
  || die "Clang C17 probe differs from the managed oracle"
...
if rg -q '^qbe-materialize OK module_0.ssa$' "$generated/report.txt"; then
  ...
  gcc "${strict[@]}" -o "$scratch/qbe_probe" "$scratch/module_0.s" \
    "$repo/native-core/shim/native_shim.c" "$here/qbe_main.c"
  "$scratch/qbe_probe" >"$scratch/qbe.out"
  cmp -s "$scratch/qbe.out" "$generated/managed.out" \
    || die "QBE probe differs from the managed oracle"
