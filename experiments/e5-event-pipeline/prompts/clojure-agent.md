# E5c Bug Detection — Clojure Agent Prompt

You are fixing bugs in a Clojure codebase.

## Task

The directory `TRIAL_DIR/` contains 8 Clojure source files that form an
...
- `bin/clj-sig FUNCTION TRIAL_DIR/` — print a function's signature (arglist)
- `bin/clj-fields RECORD TRIAL_DIR/` — print defrecord fields
- `bin/clj-callers FUNCTION TRIAL_DIR/` — find call sites
...
2. Read each module, looking for:
   ...
   - Logic errors (wrong arithmetic operator, wrong comparison, wrong filter)
...
commands, handlers, queries, pipeline, notifications, analytics. Each module
has 15-25 functions.
